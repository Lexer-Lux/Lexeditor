...
Node.js must also be on PATH. Each plugin's checks, and the shared ones, are
one command; CI runs exactly the same thing (`.github/workflows/<plugin>-checks.yml`
and `global-checks.yml`, generated by `--write-workflows`):

```powershell
.venv/Scripts/python.exe tools/check_plugin.py ff8        # one plugin
.venv/Scripts/python.exe tools/check_plugin.py --global   # shared UI and core
.venv/Scripts/python.exe tools/check_plugin.py ff8 --list # what it would run
```

A test belongs to the plugin whose folder it sits in: `tests/<plugin>/` holds
that plugin's checks, helpers and fixtures, and `tests/shared/` holds the ones
that belong to no plugin.
Run the baseline shared checks directly with:

```powershell
.venv/Scripts/python.exe tests/shared/verify_all.py verify_regressions.py verify_browser_regressions.py frontend_syntax shared_ui_contract no_clipped_text --jobs 2 --timeout 300 --retries 0
```
...
`%TEMP%/lexeditor-dev/no-clipped-text.json`; the console prints the first twenty.
...
Run the local RDR2 audit with `tests/rdr2/verify_rdr2_runtime.py`. It includes executable
