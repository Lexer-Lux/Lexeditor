...
class CoverageTests(unittest.TestCase):
    ...
    def test_data_map_includes_world_headers_navigation_and_scripts(self):
        ...
        self.assertEqual(world_row["status"], "partial")
        self.assertIn("map painting", world_row["notes"])
    ...
    def test_data_map_describes_desktop_named_event_editing(self):
        ...
        self.assertEqual(event_row["status"], "partial")
        self.assertIn("Some commands are read-only", event_row["notes"])
        self.assertIn("cannot add or remove", event_row["notes"])
        self.assertNotIn("0x", event_row["notes"])

    def test_data_map_excludes_tools_and_project_operations(self):
        store = SimpleNamespace(archive=SimpleNamespace(entries=[]))
        mapped = augment_data_map(store, {"rows": [], "counts": {"resources": 0}})
        names = [row["filename"] for row in mapped["rows"]]
        self.assertNotIn("Actual resources.bin path families", names)
        self.assertNotIn("<project>.ctp", names)
        self.assertNotIn("Selected project overlay", names)
    ...
    def test_data_map_describes_scene_render_diagnostics_without_playback_claim(self):
        ...
        self.assertIn("Map painting", row["notes"])
        ...
        self.assertIn("editing and playback are not available", animation_row["notes"])
