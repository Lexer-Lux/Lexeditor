...
class CoverageTests(unittest.TestCase):
    ...
    def test_data_map_describes_desktop_named_event_editing(self):
        ...
        self.assertIn("named fixed-width editors", event_row["controls"])
        self.assertIn("coverage audit", event_row["controls"])
        notes = event_row["notes"]
        self.assertIn("chrono_trigger_event_audit.py", notes)
        self.assertIn("real Steam archive", notes)
        self.assertIn("deduplicates aliased function bounds", notes)
        self.assertIn("excludes zero-argument commands", notes)
        self.assertIn("prioritizes parser-stop opcodes", notes)
        self.assertIn("malformed overlays", notes)
        ...
        self.assertIn("PC segment-memory 0x48–0x4D", notes)
        self.assertIn("raw u16 segment fields", notes)
        self.assertIn("no fabricated full RAM address", notes)
        ...
        self.assertIn("0x1E/1F/25/26", notes)
        self.assertIn("0x00–0x32", notes)
        self.assertIn("0x0D/0x0E", notes)
        self.assertIn("0x7A/8F/94–99/9A/9D/A0/A1/B5/B6", notes)
        self.assertIn("D9/E2/E7/F4", notes)
        self.assertIn("EB song volume", notes)
        self.assertIn("29/82/C8", notes)
        ...
        self.assertIn("blocked from raw set-args writes too", notes)
        self.assertIn("PC parser contracts govern 0x2E/0x88/0x4E", notes)
        self.assertIn("F1 stops fail-closed", notes)
        self.assertIn("0x60/61/67/75/76/77/7B/27/28/8D/8E/92/9C/9E/9F", notes)
        self.assertNotIn("0x48–4D/60", notes)
    ...
    def test_data_map_describes_scene_render_diagnostics_without_playback_claim(self):
        ...
        self.assertIn("unknown layer-priority data", row["notes"])
