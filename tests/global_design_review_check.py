...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        for width, height in [(900,620), (1600,900)]:
            ...
            page.get_by_label('Layout proposal', exact=True).select_option('properties')
            ...
            page.get_by_label('Layout proposal', exact=True).select_option('context')
            assert page.locator('.lex-review-body > .lex-review-model').count() == 1
            page.locator('.lex-design-review').evaluate('(e)=>e.scrollTop=0')
            page.screenshot(path=str(out/f'layout-{width}.png'))
            page.get_by_label('Formula typography', exact=True).select_option('inline')
            assert page.locator('.lex-review-fraction').count() == 0
            page.get_by_label('Formula typography', exact=True).select_option('stacked')
            ...
            page.close()
            results.append({'viewport':[width,height],'interactive_layouts':'pass','formula_choices':'pass','errors':errors})
