...
def check() -> int:
    ...
    problems = [f"{plugin}: {lines} copied lines, up from {recorded.get(plugin, 0)}. "
                ...
                "tests/shared/verify_shared_code_budget.py --update if this is deliberate."
...
def update() -> int:
    BUDGET.write_text(json.dumps({
        "note": "Copied lines per plugin. May fall, never rise. See tests/shared/verify_shared_code_budget.py.",
