...
def test_tests_are_one_folder_per_plugin_plus_shared():
    # tests/<plugin>/ is that plugin's (tools/check_plugin.py runs it), and
    # tests/shared/ is everything global. Nothing else sits in tests/.
    tests = ROOT / "tests"
    folders = set(check_plugin.PLUGINS) | {check_plugin.SHARED, "__pycache__"}
    loose = [p.name for p in tests.iterdir() if p.is_file() and p.name != "plugin_checks.json"]
    assert not loose, f"move {loose} into tests/<plugin>/ or tests/shared/"
    unknown = [p.name for p in tests.iterdir() if p.is_dir() and p.name not in folders]
    assert not unknown, f"tests/{unknown} match no plugin folder in plugins/; use tests/shared/ for global checks"


def test_test_file_names_are_unique_across_folders():
    # pytest imports test modules by base name; two folders with the same
    # file name break collection, and bare helper imports become ambiguous.
    seen = {}
    for p in tracked():
        if p.startswith("tests/") and p.endswith(".py") and "/fixtures/" not in p:
            seen.setdefault(Path(p).name, []).append(p)
