...
_lex_sys.path.insert(1, str(_LexPath(__file__).resolve().parents[2] / "tools"))
...
def main() -> int:
    ...
    runner = (ROOT / "tests" / "shared" / "verify_all.py").read_text(encoding="utf-8")
    ...
    listed = _sp.run([sys.executable, str(ROOT / "tests" / "shared" / "verify_all.py"), "--list"],
