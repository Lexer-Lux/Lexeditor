...
platform-specific build tools. Each attempt has a deadline and a complete log.
    ...
    python tests/shared/verify_all.py                  every verifier
    python tests/shared/verify_all.py ff8 curve        only verifiers matching a name
    python tests/shared/verify_all.py --jobs 4         fewer workers on a busy machine
"""
...
def _once(tool: Path, timeout: float = 180, output: Path | None = None,
    ...
    output = output or DEV_CACHE / "verify-results"
...
_PREPARED_SOURCE_MARKERS = (
    "\\lexeditor-dev\\ffnx-upstream\\",
    "/lexeditor-dev/ffnx-upstream/",
    "\\lexeditor-dev\\issue51-ffnx-build-",
    "/lexeditor-dev/issue51-ffnx-build-",
)
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=DEV_CACHE / "verify-results")
    ...
    # One folder per plugin plus tests/shared/; ordered by name, not folder.
    tools = [tool for tool in sorted((ROOT / "tests").glob("*/verify_*.py"), key=lambda tool: tool.name)
    ...
    timings = {}
    cache = DEV_CACHE / "verify-durations.json"
    try:
