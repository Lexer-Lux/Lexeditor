...
platform-specific build tools. Each attempt has a deadline and a complete log.
    ...
    python tests/verify_all.py                  every verifier
    python tests/verify_all.py ff8 curve        only verifiers matching a name
    python tests/verify_all.py --jobs 4         fewer workers on a busy machine
"""
...
def main() -> int:
    ...
    tools = [tool for tool in sorted((ROOT / "tests").glob("verify_*.py"))
