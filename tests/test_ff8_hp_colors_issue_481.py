...
class BetterHpColorsTests(unittest.TestCase):
    ...
    def test_runtime_config_can_enable_then_restore_vanilla(self):
        with tempfile.TemporaryDirectory(prefix="ff8-hp-colors-") as directory:
            ...
            gameplay_settings._set_ffnx_runtime_tweaks(
                config, better_hp_colors=True, **common
            )
            ...
            gameplay_settings._set_ffnx_runtime_tweaks(
                config, better_hp_colors=False, **common
            )
    ...
    def test_editor_exposes_default_off_control_and_semantics(self):
    ...
    def test_native_candidate_keeps_switch_default_off(self):
