...
spec=importlib.util.spec_from_file_location('duration_catalog',Path(__file__).resolve().parents[2]/'tools/probes/rdr2_duration/catalog.py')
...
class DurationProbe(unittest.TestCase):
 ...
 def test_probe_and_mod_restore_preserve_exact_bytes(self):
  import tempfile
  spec=importlib.util.spec_from_file_location('duration_deploy',Path(__file__).resolve().parents[2]/'tools/probes/rdr2_duration/deploy.py');d=importlib.util.module_from_spec(spec);spec.loader.exec_module(d)
  with tempfile.TemporaryDirectory() as temp:
 ...
 def test_activation_failures_roll_back_and_repeats_are_bounded(self):
  ...
  from unittest.mock import patch
  sys.path.insert(0,str(Path(__file__).resolve().parents[2]/'tools/probes/rdr2_duration'))
 ...
 def test_changed_prepared_probe_has_zero_live_writes(self):
  import tempfile,sys,importlib
  from unittest.mock import patch
  sys.path.insert(0,str(Path(__file__).resolve().parents[2]/'tools/probes/rdr2_duration'));c=importlib.import_module('coordinator')
  with tempfile.TemporaryDirectory() as temp:
 ...
 def test_incomplete_probe_stage_never_becomes_live(self):
  import tempfile,sys,importlib
  from unittest.mock import patch
  sys.path.insert(0,str(Path(__file__).resolve().parents[2]/'tools/probes/rdr2_duration'));d=importlib.import_module('deploy')
  with tempfile.TemporaryDirectory() as temp:
