...
class BannerlordPluginTests(unittest.TestCase):
    ...
    def test_full_submodule_editing_preserves_unknown_data(self):
        from plugins.bannerlord.module_data import data_map,read_submodule,save_module
    ...
    def test_msbuild_properties_source_and_build_helpers(self):
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            with patch("plugins.bannerlord.project_data.subprocess.run",return_value=completed) as runner:build=run_build(project,configuration="Release")
    ...
    def test_text_preserving_project_skill_and_effect_writers_keep_bom_crlf(self):
        ...
        from plugins.bannerlord.skill_data import read_effect_definitions, read_skill_definitions, save_effect_definitions, save_skill_definitions
        with tempfile.TemporaryDirectory() as name:
