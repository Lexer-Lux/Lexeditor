...
- The 150% browser geometry mirrors the real Windows host: `windows_host.set_ui_scale`
  ...
  into 150vh).
- [ ] scoped rendered browser evidence must pass on the final head and its
  screenshots must be inspected before this audit is considered complete.
- [ ] refresh the isolated installed Windows candidate only after that rendered
  gate passes.
