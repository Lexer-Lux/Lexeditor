...
Candidate built and held (out/rdr2-after-duration, not installed while #151 owns the game files). Eagle Eye correction installed; horse recovery unproven. The acceptance procedure in this handoff stands as written (level dry ground, exhaust outer ring, slow without core use, 18 percent latch release, gait/stop/dismount/water checks). No code change; needs install plus game run. Issue stays actionable.
