...
- `FFXDataParser` is a research cross-check for the FFX `u16` fixed-record container, the FFX-2 `u32` fixed-record container, and the proved record fields exposed here. Its source is not copied into Lexeditor.
...
- First FFX-2 structured editor: `new_uspc/battle/kernel/command.bin`, writing only animation IDs at record offsets `+0x08/+0x0A`; string refs, unknown bytes and trailing localized strings are read-only/preserved.
...
- Eight shared-shell structured views (seven FFX + one FFX-2) plus archive browser, Data Map and install/project/deployment status.
...
- Expand FFX-2 only through independently proved record fields; localized string editing and other command fields remain intentionally untouched.
