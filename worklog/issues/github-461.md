...
- FFXDataParser also identifies `new_uspc/battle/kernel/a_ability.bin` as `0x6C` records and maps the five element bytes at `+0x11..+0x15` as Strike, Absorb, Immune, Resist and Weak.
...
- **Twelve conservative FFX structured tables**:
...
- The auto-ability element editor exposes only five known element bits per behavior. Save logic performs `(old & 0xE0) | new_known_mask`, preserving unknown high bits even inside the five editable source bytes.
...
- All **14 structured tables** use exact VBF-header MD5 plus exact table SHA-256 stale-write protection.
...
- Managed auto-ability coverage saves only to canonical `efl/x/FFX_Data/.../a_ability.bin` and verifies the installed VBF SHA-256 is unchanged.
...
- Do not widen `a_ability.bin` merely because more bytes have labels in one source; only independently agreed semantics should become writable.
