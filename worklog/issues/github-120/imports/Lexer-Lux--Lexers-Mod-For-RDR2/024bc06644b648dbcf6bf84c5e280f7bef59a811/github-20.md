...
## 2026-09-06 shared-development startup repair (Lexeditor #120)

Primary evidence: current central issue #120 and preserved comments require a
development build to start with the common readouts enabled. script.cpp
initializes g_runtimeDevelopmentMode to false unconditionally, contradicting
that requirement and the existing verify_dev_mode_issue_20.py contract. Read
fuckups.txt; recurrence risks are gating gameplay on a diagnostic flag and
claiming a compiled build proves the UI. The one-line candidate initializes
from GameplayTweaksBuild::Development; it does not add gameplay gates, restore
ShowMode, or change Tilde behavior. Execution proof will compile the actual
initializer with both release and development headers and run the toggle.
In-game readouts and release/development gameplay parity remain unverified.
