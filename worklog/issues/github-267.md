...
No new code: the module comment block records the honest model (no engine shoulder-side setter on the native surface; offset is magnitude-only on the engine-owned side), and the log already records press vs rendered lateral for falsification. Settled left/right equality can only be settled by measurement.
Human test, needs built ASI on a game machine: aim, press X, screenshot settled left; press X, screenshot settled right; measure Arthur screen fraction in both. Report both fractions with the log press lines. The latest retest failed, so centerline crossing alone is not success. Issue stays actionable.
