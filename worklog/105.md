...
Read belt_lantern.cpp fully (482 lines): prop attachment, radial light toggle, mission/death/swim/ragdoll gates all present, zero crouch-state handling and zero prior-state restore. The gate pattern is ready, but no stance/crouch signal source is established anywhere in tree (stance matches are distance/instance substrings). No native was guessed. Crouch-dim plus standing-restore needs game proof of the crouch signal first, then the candidate. Issue stays actionable.
