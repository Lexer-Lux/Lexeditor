...
- Gap rows stay explicit in `games/ff9/server.py` `UNRESOLVED_AREAS` and are now
  locked visible-but-closed by `tests/ff9/test_ff9_datamap_gui.py` (#525).
...
- Repaired `tests/ff9/test_ff9_datamap_gui.py`: it still imported `games.ff9` and
...
- Evidence, worktree at `origin/master` `7bf81c04`: FF9 pytest files
  ...
  — 158 passed; `node --test tests/ff9/ff9_editor.test.cjs` — 18 passed;
...
- Agent-side verification at this head, all green: FF9 pytest files (test_ff9_csv/battle_scene/datamap_gui/features/field_walkmesh/http/mod_compat/runtime/runtime_source) - 158 passed; node --test tests/ff9/ff9_editor.test.cjs - 18 passed; app.py --list / --game ff9 --check (ff9: ready) / --game ff9 --smoke (5 PASS); tools/generate_credits.py --check pass; tools/ff9/patch_features_ui.py --check PASS; tests/ff9/ff9_browser_check.py rendered acceptance passed. Catalog still reports all 42 pinned Memoria CSV datasets.
