...
- `python tests/verify_ff7_datasets.py`: 19 passing local synthetic binary/HTTP tests, including every exposed character field in every slot.
- `python tests/verify_ff7_extended.py`: 15 passing local tests for compression, byte preservation, text controls, documented offsets, invalid references, stale files, backups, unsupported profiles, source isolation and archive overflow.
