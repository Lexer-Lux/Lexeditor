...
Updated 2026-09-08 for the completion pass on PR #434. This is the current scope reference; the earlier `recovery-20260906.md` is a historical session record, not the current list of missing editors. Both `ff7` and `ff7-2013` use `plugins.ff7.server` and the same page. No other game's implementation is changed by this pass.
...
LGP editing retains lookup/conflict tables, other members and inactive bytes. A resized/aliased member is appended before the footer and its TOC pointer updated, as allowed by the documented format. Installed archives use the 14-byte `FINAL FANTASY7` footer (the 15-byte form stays accepted for Lexeditor-written archives and each archive keeps its own footer). Only extensionless members are PC field files; dotted members (textures, tutorial data, console-port variants) are preserved but not encounter sources. Only the encounter section's slot must be tight: vanilla 2013 fields (blackbgb, fship_4, las0_8) carry a loose size elsewhere and the engine navigates by pointers. Field editing touches only section 7's two 24-byte encounter tables. World enc_w.bin remains 2208 bytes. IDs and weights are separated from their packed representation. An enabled zero encounter rate and invalid normal probability totals are refused.

Executable editing is restricted to the five recognized English SHA-1 profiles also accepted by Scarlet 10a2283. The raw 0x200/0x400 build shift is applied to Scarlet's named offsets; no per-build offsets are guessed. A project executable may differ from its installed source only inside the explicitly modeled ranges. Fixed text retains original padding/control bytes when unchanged and refuses oversized encoded strings. The 46 Chocobo name slots and unused shop text slot 10 are NUL-padded C strings rather than 0xFF-terminated; edits preserve each slot's convention. The AP multiplier's two executable copies are written together. Opaque world-model walk/disembark instruction bitmasks remain outside the ordinary first-class surface rather than being presented as unexplained settings.
