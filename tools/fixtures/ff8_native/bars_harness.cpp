...
#define IM_COL32_A_MASK (0xFFu << IM_COL32_A_SHIFT)
...
struct game_obj {};
struct TestVertexColor { union { std::uint32_t color; struct { std::uint8_t b,g,r,a; }; }; };
...
struct ff8_indexed_vertices {
    ...
    nvertex *vertices=nullptr;std::uint32_t indexcount=0,field_1C=0;
    std::uint16_t *indices=nullptr;std::uint32_t field_24=0;
    unsigned char *palettes=nullptr;void *graphics_object=nullptr;
};
constexpr int TEXTCOLOR_YELLOW=0,TEXTCOLOR_WHITE=1;
...
static void common_draw_paletted2D(polygon_set *,indexed_vertices *iv,game_obj *) {
    ++paletted_draw_calls;auto *native=reinterpret_cast<ff8_indexed_vertices *>(iv);
    observed_palettes.assign(native->palettes,native->palettes+native->count);
    observed_colors.clear();
    for(std::uint32_t i=0;i<native->vertexcount;++i) observed_colors.push_back(native->vertices[i].color.color);
}
