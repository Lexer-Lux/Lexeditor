...
with tempfile.TemporaryDirectory() as td:
 ...
 assert find_original_location('a.bin',[cache],game)==target.resolve()
 ...
 assert find_original_location('ff8/en/exe/card_names.msd',[cache],game)==(game/'FF8_EN.exe').resolve()
...
with tempfile.TemporaryDirectory() as td:
 ...
 assert find_original_location('*.xml',[prepared],game)==prepared.resolve()
 assert find_original_location('a.xml + b.xml',[prepared],game)==prepared.resolve()
 archive=game/'content.rpf';archive.write_bytes(b'archive')
 assert find_original_location('content.rpf:/missing/member.xml',[prepared],game)==archive.resolve()
 assert find_original_location('game/content.rpf:/missing/member.xml',[prepared],game)==archive.resolve()
 (game/'game').mkdir();installed_archive=game/'game'/'content.rpf';installed_archive.write_bytes(b'archive')
 assert find_original_location('game/content.rpf:/missing/member.xml',[prepared],game)==installed_archive.resolve()
