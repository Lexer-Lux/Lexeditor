        ...
        public static byte[] DecompressFile(byte[] bytes, int usize, Compressorid compressorid)
        {
            switch (compressorid)
            {
                case Compressorid.None:
                    Array.Resize(ref bytes, usize);
                    return bytes;

                case Compressorid.Deflate:
                    return DeflateDecompress(bytes);

                case Compressorid.Oodle:
                    return OodleLZ_Decompress(bytes, usize);
                default:
                    throw new Exception("unkown compression type: " + compressorid);
            }
        }
