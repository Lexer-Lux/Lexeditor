  ...
  function unavailable(key){const meta=catalogRow(key),data=state.datasets[key],battle=["enemies","encounters"].includes(key);return detailPanel({
    ...
    LexeditorUI.notice({tone:"warning",title:"Source currently unavailable",
      message:data?.error||(battle?"The installed p0data2.bin battle-scene source is not available right now.":"The verified Memoria baseline source could not be loaded right now.")}),
    detailSection({title:"PROJECT",body:[
      LexeditorUI.detailNote(battle?"The format-specific enemy and encounter editor is integrated, but it needs an installed FF9 p0data2 source to enumerate battle scenes. Saves write only Memoria raw16 project overlays.":"This editor format is integrated. Reopening it retries the verified Memoria baseline; saves still write only to the selected project overlay and never overwrite the installed-game baseline."),
