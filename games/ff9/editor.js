  ...
  function unavailable(key){const meta=catalogRow(key),data=state.datasets[key],battle=["enemies","encounters"].includes(key),walkmesh=key==="field-walkmesh";return detailPanel({
    ...
    detailSection({title:"PROJECT",body:[
      LexeditorUI.detailNote(battle?"The format-specific enemy and encounter editor is integrated, but it needs an installed FF9 p0data2 source to enumerate battle scenes. Saves write only Memoria raw16 project overlays.":walkmesh?"The walkmesh editor is a deliberately partial integration: it edits only the documented floor-active bit and writes a canonical loose .bgi.bytes project overlay while preserving every other byte.":"This editor format is integrated. Reopening it retries the verified Memoria baseline; saves still write only to the selected project overlay and never overwrite the installed-game baseline."),
