"use strict";
  ...
  const state={tab:"items",modOnly:false,reshade:null,dashboard:null,dataMap:null,catalog:[],datasets:{},datasetChoice:{},activeSource:"mine",selected:{},query:{},page:{},pageSize:{},sort:{},mapQuery:"",mapStatus:"",mapPage:0,mapSort:["filename",1],busy:false,error:"",runtimeError:"",tweak:"memoria",features:null,savedFeatures:null,deployment:null};
  const prefCache={};
  ...
  function columnsFor(data,key){const useful=data.fields.filter(field=>!["id","comment","name"].includes(field.key.toLocaleLowerCase())).slice(0,3),columns=[{key:"name",label:"Name",sortable:true,width:"minmax(9em,1.6fr)"},...useful.map(field=>({key:field.key,label:field.label,sortable:true,width:"minmax(0,1fr)",numeric:["integer","number"].includes(field.kind),sortValue:row=>row.values[field.key],render:row=>fieldValue(row,field)}))];if(sourceHasId(data))columns.unshift({key:"id",label:"ID",numberedId:data.rows.every(row=>row.id===null||row.id===undefined||/^[-+]?\\d+$/.test(String(row.id))),sortable:true});return columns}
  ...
  function fieldControl(data,row,field){
    ...
    if(note||!field.editable||field.kind==="stored")control=readonlyField(String(row.values[field.key]??""),{className:"ff9-readonly"});
    ...
    return detailField({label:field.label.toLocaleUpperCase(),help:semantic?infoHelp(semantic):null,control,dataType:note?"READ ONLY":field.declaredType,min:bounds.min,max:bounds.max});
  }
  ...
  function detail(data,row){const identity=sourceHasId(data)?recordId(row.id):null;if(data.key==="items")return detailPanel({className:"ff9-detail",title:row.name,identity,meta:`Items · ${data.source} CSV`,body:[itemSections(data,row)]});const visible=data.fields.filter(field=>!["id","comment"].includes(field.key.toLocaleLowerCase())),editable=visible.filter(field=>field.editable&&field.kind!=="stored"&&!readOnlyNote(data,field)),stored=visible.filter(field=>!field.editable||field.kind==="stored"||readOnlyNote(data,field));const body=[];if(editable.length)body.push(detailSection({title:"EDITABLE DATA",body:editable.map(field=>fieldControl(data,row,field))}));if(stored.length)body.push(detailSection({title:"STORED DATA",body:stored.map(field=>fieldControl(data,row,field))}));return detailPanel({className:"ff9-detail",title:row.name,identity,meta:`${data.label} · ${data.source} CSV`,body})}
  function unavailable(key){const meta=catalogRow(key),data=state.datasets[key],battle=["enemies","encounters"].includes(key);return el("section",{class:"ff9-card"},el("h2",{},data?.label||meta?.label||tabs.find(tab=>tab.id===state.tab)?.label||"FF9 data"),el("p",{},data?.error||(battle?"The installed p0data2.bin battle-scene source is not available right now.":"The verified Memoria baseline source could not be loaded right now.")),el("p",{},battle?"The format-specific enemy and encounter editor is integrated, but it needs an installed FF9 p0data2 source to enumerate battle scenes. Saves write only Memoria raw16 project overlays.":"This editor format is integrated. Reopening it retries the verified Memoria baseline; saves still write only to the selected project overlay and never overwrite the installed-game baseline."),el("code",{class:"ff9-path"},data?.sourcePath||meta?.projectPath||state.dashboard.project.root),el("span",{class:"ff9-warning"},"SOURCE CURRENTLY UNAVAILABLE"))}
  function statusPanel(title,meta,message){return detailPanel({className:"ff9-detail",title,meta,body:[detailSection({title:"STATE",body:[detailField({label:"STATUS",control:readonlyField(message)})]})]})}
  function renderDataset(data,key){if(!data||data.unavailable||data.available===false){$("#main").replaceChildren(unavailable(key));return}if(!data.rows.length){$("#main").replaceChildren(statusPanel(data.label,`${data.source} CSV`,"This source is valid but contains no records."));return}const rows=viewRows(data,key),selected=rows.find(row=>row.line===state.selected[key])||rows[0];if(selected)state.selected[key]=selected.line;const layout=pagedListDetail({modOnly:modOnlySpec(key,row=>rowChanged(data,row)),rows,key:row=>row.line,slots:true,selected:selected?.line??null,page:state.page[key]||0,pageSize:state.pageSize[key]||12,noun:"records",className:"ff9-layout",splitKey:`ff9-${state.tab}-${key}`,rowsKey:`ff9-${state.tab}-${key}`,defaultSplit:44,minLeft:340,minRight:420,search:{key:`ff9-${key}`,value:state.query[key]||"",label:`Search ${data.label}`,change:value=>{state.query[key]=value;state.page[key]=0;render()}},sync:next=>{state.page[key]=next.page;state.pageSize[key]=next.pageSize;if(next.selected!==null)state.selected[key]=next.selected},change:next=>{state.page[key]=next.page;state.pageSize[key]=next.pageSize;if(next.selected!==null)state.selected[key]=next.selected;render()},master:view=>tablePanel(data,key,view.rows,view.selected,view.select),detail:row=>detail(data,row)});$("#main").replaceChildren(layout)}
