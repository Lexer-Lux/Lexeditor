  ...
  function unavailable(key){const meta=catalogRow(key);const data=state.datasets[key];return detailPanel({
    title:meta?.label||tabs.find(tab=>tab.id===state.tab)?.label||"FF9 data",body:[
    LexeditorUI.notice({tone:"warning",title:"Source currently unavailable",
      message:data?.error||"The verified source data could not be loaded right now."}),
    detailSection({title:"PROJECT",body:[
      LexeditorUI.detailNote("This editor format is integrated. Reopening it retries the verified Memoria baseline; saves still write only to the selected project overlay and never overwrite the installed-game baseline."),
      detailField({label:"FOLDER",control:readonlyField(meta?.projectPath||state.dashboard.project.root,{format:false})})]})]})}
