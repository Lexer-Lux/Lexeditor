  ...
  async function boot(){
    try{[state.files,state.dashboard,state.items,state.shops,state.strings,state.missions,state.settings,state.loot,state.vanilla.items,state.vanilla.shops,state.vanilla.strings,state.vanilla.missions]=await Promise.all([api("/api/files"),api("/api/dashboard"),api("/api/items"),api("/api/shops"),api("/api/string-tables"),api("/api/missions"),optionalRuntime("/api/settings"),optionalRuntime("/api/loot"),api("/api/items?dataset=vanilla"),api("/api/shops?dataset=vanilla"),api("/api/string-tables?dataset=vanilla"),api("/api/missions?dataset=vanilla")]);state.lootDocument=state.loot.available?clone(state.loot.document):null;const firstLanguage=stringsUI.firstLanguageId();if(firstLanguage)await stringsUI.load(firstLanguage,false);state.booting=false;render();if(!state.dashboard.redHook.installed)renderRedHookNotice();LexeditorUI.finishPluginLoading();}
