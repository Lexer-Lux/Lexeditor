...
function itemDetail(){
  ...
  return LexeditorUI.detailPanel({className:"record-detail item-detail",
    ...
    body:[
      detailField("Type",item.type,"","The XML element this record comes from. It decides which fields the game reads."),
      detailField("Dataset",item.sourceLabel,"","Which shipped data file this item was defined in: the base game or one of its DLC."),
      detailField("Source",el("code",{title:item.sourcePath},item.sourcePath),"","The untouched file inside the installed content.rpf. Lexeditor never writes here."),
      detailField("Override",el("code",{title:item.projectPath},item.projectPath),"","Where your edit is written, as a full XML override. The game loads this instead of the vanilla file."),
...
function shopDetail(){
  ...
  return el("div",{class:"lex-detail record-detail shop-detail"},
    ...
    detailField("Shop",item.shop),detailField("Type",item.category),
    detailField("Root",el("code",{},item.rootHash)),
    detailField("Source",el("code",{title:item.sourcePath},item.sourcePath)),
    detailField("Override",el("code",{title:item.projectPath},item.projectPath)),
...
function missionDetail(){
  ...
  return el("div",{class:"lex-detail record-detail mission-detail"},
    ...
    detailField("ID",String(mission.id)),detailField("Script",mission.scriptName),detailField("Area",missionArea(mission)),
    detailField("Text key",el("code",{},mission.localizationKey)),detailField("Source",el("code",{title:mission.archivePath},mission.archivePath)),
    detailField("Evidence",el("code",{title:mission.rewardSource.file},`${mission.rewardSource.function} / ${mission.rewardSource.case}`)),
...
function renderLoot(){
  ...
  const scope=el("section",{class:"card"},
    el("h2",{},"What this tab covers",
      infoHelp("Searched, not assumed. content.rpf holds the corpse loot logic at content/release64/scripting/gringo/commonscripts/lootcorpsegenericnoanim.wsc. What a body yields is a switch on its LootType decorator: each verified branch adds a fixed item enum in Function_90. The table on this page rewrites only those proven item-enum push operands with a length-preserving script override. None of the other WSC bytecode is exposed as editable, and none of the archive XML files is treated as a drops table.")),
...
function renderProject(){
  ...
  const statusCard=el("section",{class:"card"},el("h2",{},"Preparation"),state.dashboard.problems.length?el("div",{class:"error"},...state.dashboard.problems.map(problem=>el("div",{},problem))):el("div",{class:"ok"},`${state.files.counts.all||0} tuning files, ${state.items?.counts?.all||0} items, ${state.shops?.counts?.items||0} shop entries, and ${state.strings?.counts?.records||0} localized strings prepared. Installed archives are read-only.`),el("pre",{class:"manifest"},Object.keys(sources).length?`${Object.entries(sources).map(([label,source])=>`${label}: ${source.path}\nSHA-256: ${source.sha256}`).join("\n\n")}\n\nPrepared: ${state.dashboard.manifest.preparedAt}\nInventory files: ${state.dashboard.manifest.fileCounts?.inventory||0}\nString tables: ${state.dashboard.manifest.fileCounts?.stringTables||0}\nShop dictionaries: ${state.dashboard.manifest.fileCounts?.gringoUnpacked||0}`:"Preparation manifest is not available."));
  const redHook=state.dashboard.redHook;
  const intro=redHook.skipIntroLogos;
  const redHookCard=el("section",{class:"card"},el("h2",{},"RedHook"),redHook.installed?el("div",{class:"ok"},"RedHook is installed."):el("div",{class:"error"},`RedHook is required. Missing: ${redHook.missing.join(", ")}.`),redHook.installed?(intro.enabled?el("div",{class:"ok"},"Startup logo movies are disabled."):el("div",{class:"error"},intro.problem||"Startup logo skipping is not enabled.")):null,el("div",{class:"path-row"},el("b",{},"Game root"),el("code",{title:redHook.gameRoot},redHook.gameRoot)),!redHook.installed?el("button",{type:"button",onclick:()=>openRedHook()},"Open official RedHook page"):!intro.enabled?el("button",{type:"button",onclick:()=>configureRedHook()},"Enable logo skipping"):null);
  ...
  const deliveryCard=el("section",{class:"card"},el("h2",{},"Saved files and game delivery"),
    ...
    el("p",{},"LexerRDR.ini, loot JSON and mission JSON are consumed by the native runtime from this workspace; archive-backed XML, WGD, STRTBL, and verified WSC edits use the update-folder copies below."),
...
async function boot(){
  try{[state.files,state.dashboard,state.items,state.shops,state.strings,state.missions,state.settings,state.loot,state.vanilla.items,state.vanilla.shops,state.vanilla.strings,state.vanilla.missions]=await Promise.all([api("/api/files"),api("/api/dashboard"),api("/api/items"),api("/api/shops"),api("/api/string-tables"),api("/api/missions"),optionalRuntime("/api/settings"),optionalRuntime("/api/loot"),api("/api/items?dataset=vanilla"),api("/api/shops?dataset=vanilla"),api("/api/string-tables?dataset=vanilla"),api("/api/missions?dataset=vanilla")]);state.lootDocument=state.loot.available?clone(state.loot.document):null;const firstTable=stringsUI.firstAvailableId();if(firstTable)await stringsUI.load(firstTable,false);state.booting=false;render();if(!state.dashboard.redHook.installed)renderRedHookNotice();LexeditorUI.finishPluginLoading();}
