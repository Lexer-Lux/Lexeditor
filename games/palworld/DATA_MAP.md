...
Current scope targets **Palworld Windows Steam v0.7+ official mod packages** plus a conservative PalSchema raw-table editor and clean package builder. Installed game data stays read-only.
...
| PAK resource replacements | `<package>/Paks/**/*.pak` | **Recognized, unsupported** | Package/build support only; no claim of PAK asset parsing yet. |
| UE4SS Lua mods | package targets using `Type=Lua` | **Recognized, unsupported** | Loader/package/build shape recognized; no generic Lua runtime editor. |
...
| Loader activation config | `<Palworld>/Mods/PalModSettings.ini` | **Known runtime config, not project data** | Official loader owns enable state. Editing/activation remains deferred until an ownership-aware runtime path is proven. |
| Steam Workshop item directory | `steamapps/workshop/content/1623730/<item-id>/` | **Known deployment destination, not written** | The clean build is intended for Pocketpair's official uploader/Workshop flow; Lexeditor does not overwrite Steam-managed item directories or publish items in this slice. |
...
Validate package + known PalSchema raw payloads -> enumerate only official package inputs -> reject missing targets/symlinks -> exclude `.lexeditor.bak`/temporary artifacts -> build atomically to an owned snapshot -> hash/track every output file -> refuse overwrite/revert after external modification -> revert only the unchanged Lexeditor-owned snapshot. This is a packaging boundary, not activation or native-game acceptance.

## Explicitly not claimed yet

- no automatic Workshop activation or direct write to Steam Workshop item folders;
