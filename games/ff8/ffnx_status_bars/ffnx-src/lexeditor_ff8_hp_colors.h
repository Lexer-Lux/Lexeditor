...
inline LexeditorFf8HpRgb lexeditor_ff8_hp_rgb(
...
inline bool lexeditor_ff8_hp_should_tint(
    std::uint32_t current, std::uint32_t maximum)
{
    // Zero is the mathematical red endpoint, but live KO must keep FF8's
    // native presentation. Full HP is also left completely native/white.
