...
const {
  el, columnList, columnPreferences, detailField, detailPanel, detailSection,
  infoHelp, pagedListDetail, panelLayout, readonlyField, unitField,
  booleanMark,
} = LexeditorUI;
...
function sourceState() {
  ...
  return el("div", {class: "factorio-state"},
    el("section", {class: "factorio-state-card"},
      el("h2", {}, source.error ? "Prototype dump could not be read" : "Prototype dump required"),
      el("p", {}, message),
      el("p", {}, "Lexeditor will not execute source-mod Lua. The dump stays read-only; edits are stored in overrides.json."),
      el("p", {}, el("code", {}, source.path || "source/data-raw-dump.json")),
      el("div", {class: "factorio-actions"},
...
function loadingState(message = "Loading Factorio project…") {
  return el("div", {class: "factorio-state", "aria-busy": "true"},
    el("section", {class: "factorio-state-card"},
      el("h2", {}, message),
      el("p", {}, "Reading the project through the Factorio plugin service.")));
}
...
function referenceList(values, kind = "", empty = "None") {
  ...
  if (!list.length) return el("div", {class: "factorio-reference-list"},
    el("span", {class: "factorio-empty-ref"}, empty));
  return el("div", {class: "factorio-reference-list"}, ...list.map(value => {
    ...
    return el("span", {class: "factorio-reference", title: label},
      canOpen
        ? el("button", {type: "button", onclick: () => navigateTo(kind, name)}, label)
        : label);
  }));
}
...
function itemPanel(row) {
  ...
  return detailPanel({
    className: "factorio-detail", title: row.name, identity: null,
...
function prerequisiteControl(row) {
  ...
  const select = el("select", {
    class: "factorio-multi", multiple: true, size: 7,
...
function unavailableCellEditor(message, commit) {
  const control = el("button", {
    type: "button",
    class: "factorio-cell-unavailable",
    title: message,
...
function recordsPanel(kind) {
  ...
  return pagedListDetail({
    ...
    emptyDetail: () => el("div", {class: "factorio-state"},
      el("section", {class: "factorio-state-card"},
        el("h2", {}, `No ${LABELS[kind].toLocaleLowerCase()} match`),
        el("p", {}, "Clear the search or turn off Mod contents only."))),
  });
}
...
function diagnosticsSection() {
  ...
  return detailSection({title: `VALIDATION · ${rows.length}`, body: rows.slice(0, 12).map(row =>
    el("div", {class: "factorio-diagnostic", "data-severity": row.severity},
      `${row.severity.toUpperCase()} · ${row.record}: ${row.message}`))});
}
...
function infoPanel() {
  ...
  const buttons = el("div", {class: "factorio-actions"},
  ...
  const exportResult = state.exportResult
    ? el("div", {class: "factorio-export-result"},
        el("strong", {}, state.exportResult.filename),
        el("div", {}, `SHA-256: ${state.exportResult.sha256}`),
        el("div", {}, state.exportResult.message))
...
async function renderDataMap() {
  ...
  return el("div", {class: "factorio-map"},
    el("div", {class: "factorio-map-controls"}, ...view.controls),
    view.content);
}
...
async function render() {
  ...
  try {
    ...
    else if (state.tab === "info") content = panelLayout([infoPanel()], "factorio-layout", {
