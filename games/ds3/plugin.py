...
def check() -> list[str]:
    ...
    for table in (
        "EquipParamWeapon", "EquipParamProtector", "EquipParamAccessory",
        "Magic", "SpEffectParam", "NpcParam",
    ):
        required.extend([
            PLUGIN_ROOT / "metadata" / "defs" / f"{table}.xml",
            PLUGIN_ROOT / "metadata" / "meta" / f"{table}.xml",
            PLUGIN_ROOT / "metadata" / "layouts" / f"{table}.json",
            PLUGIN_ROOT / "metadata" / "row_names" / f"{table}.json",
        ])
    return [
