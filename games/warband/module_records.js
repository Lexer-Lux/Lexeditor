(() => {
  ...
  function create(options){
    ...
    function render(){
      ...
      if(entry.error){main().replaceChildren(LexeditorUI.el("section",{class:"card warband-module-state error"},LexeditorUI.el("h2",{},labelFor(active)+" could not be loaded"),LexeditorUI.el("p",{},entry.error),LexeditorUI.el("button",{type:"button",onclick:()=>load(active,true)},"Retry")));return;}
      const data=entry.data;
      if(!data?.available){main().replaceChildren(LexeditorUI.el("section",{class:"card warband-module-state"},LexeditorUI.el("h2",{},labelFor(active)+" source is unavailable"),LexeditorUI.el("p",{},"The selected project does not contain this Module System source file.")));return;}
