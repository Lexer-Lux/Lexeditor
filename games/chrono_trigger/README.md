...
- Scene MapTable/collision inspection plus read-only PC render diagnostics.
  ...
  - PC `PrioMap` bytes are displayed raw; CTViewer itself still describes the four per-map bytes as unknown PC-only layer-priority data, so their runtime semantics remain explicitly unclaimed.
...
- Atel parsing, PC disassembly, semantics and control-flow diagnostics, with named fixed-width argument editing in both the desktop Events UI and CLI. Unsupported, variable-width and unresolved commands remain read-only.
  ...
  - Script-memory assignment/math controls include `0x19`, `0x1A`, `0x4F`/`0x50`, `0x51`/`0x52`, `0x5B`, `0x5D`/`0x5E`, `0x5F`, and `0x71`/`0x72`/`0x73`; these use the validated even `0x7F0200`–`0x7F03FE` `/2` address model. `0x1C` Get Result is a separately proven narrow bank-7F form whose one operand is a literal byte offset from `0x7F0000`, so Lexeditor exposes only the actually encodable `0x7F0000`–`0x7F00FF` range.
  ...
  - Proven doubled-object controls `0x0A`, `0x0B`, `0x0C`, `0x7C`, `0x7D` edit only the even `objectId*2` target byte. For `0x0B`/`0x0C`, the immutable opcode continues to determine processing off/on; the fixed-field editor cannot switch modes.
  - Facing-target controls `0xA8`/`0xA9` use an even doubled object/PC target byte. `0x23`/`0x24` Get Facing use the same doubled target plus a `/2` script-memory result destination. Odd stored targets remain read-only and `0x7F0400` is rejected because the one-byte result slot ends at `0x7F03FE`.
  ...
  - Additional movement/follow editors include `0x8F`, `0x94`/`0x95`, `0x96`/`0x97`, `0x98`/`0x99`, `0x9A`, `0x9D`, `0xA0`/`0xA1`, and looping `0xB5`/`0xB6`. `0x9D` uses two `/2` script-memory sources for direction and magnitude. Where upstream establishes only raw X/Y bytes, Lexeditor labels them as coordinate bytes rather than inventing tile/pixel units. Direct vector `0x92`/`0x9C` stay read-only because upstream's constructor doubles magnitude while its menu decode does not undo that; `0x9E`/`0x9F` remain excluded because upstream width metadata is internally inconsistent.
  ...
  - Audio includes proven `0xEB` Song Volume as `[duration, volume]`, with `0xFF` documented upstream as normal volume. `0xEC` is **not writable**: its live Sound menu proves subcommand-dependent widths, so Lexeditor decodes known EC forms dynamically (1/2/3 argument bytes) and exposes exact read-only semantic labels, while unknown/truncated forms stop fail-closed.
  ...
  - Existing proven fixed-width controls also include script-memory outputs `0x20`/`0x55`/`0x7F`, coordinate reads `0x21`/`0x22`, palette `0x33`, storyline `0x5A`, player/party management controls already in the base editor, LoadEnemy/solidity `0x83`/`0x84`, Script/NPC movement `0x87`/`0x89`/`0x8A`/`0x8B`/`0x8C`, direct facing `0xA6`/`0xA7`, animation IDs `0xAA`/`0xAB`/`0xAC`/`0xB7`, pause ticks `0xAD`, message-table selection `0xB8`, Explore Mode `0xE3`, sound/music IDs and raw darken duration `0xF0`.
  ...
  - Ambiguous commands stay out of the named editor layer: current evidence is insufficient for `0x8D` pixel-position shift behavior, `0x8E` priority/unknown flags, `0x27`/`0x28` target normalization, `0x16` bank-7F comparison packing, PC-only `0x6E` extended-memory comparison semantics, `0x67` reset-mask polarity, `0x61` operation width (upstream literally labels it `1 byte?`), direct-vector `0x92`/`0x9C` magnitude semantics, `0x9E`/`0x9F` movement widths/targets, and variable-width `0xEC` writes.
...
- Event-editor regressions cover PC item/category layouts, script-memory `/2` address round-trips, the narrow `0x1C` bank-7F result byte-offset range, doubled target IDs, function-call priority/function packing, facing target/result controls, processing targets, local bit masks/shifts, raw-slot PC-only extended-memory operations, known property-bit writes with unknown-bit preservation, E2 location-from-memory addresses, party/screen controls, raw-coordinate movement/follow commands including `0x9D` vector-memory sources, EB song volume, canonical ASCII index encoding, raw NPC/dialog operands, palette/storyline/raw-solidity, movement/facing/animation/pause controls, storyline/button conditionals, 8/16-bit immediate and memory-to-memory comparisons, safe relative-jump retargeting, fixed-size preservation, partial edits, and fail-closed invalid encodings.
- Disassembly regressions cover every live Sound-menu EC width class: subcommands `88/F0/F2` = one argument byte, `14/19` = two, and `82/83/85/86` = three; known EC forms also receive read-only semantic summaries, unknown/truncated forms fail closed, and EB remains fixed at two argument bytes.
...
- Playwright covers scene/world raster views and render diagnostics plus real Events UI writes: the main regression edits `NPC Facing` and a `0x13` 16-bit comparison with stale-SHA rollover; dedicated regressions cover `0x6B` Toggle Bits, PC-only extended raw-slot editing, and a three-save sequence for `0x23` Get Facing, `0x0B` processing target and `0x9D` vector-memory sources with exact refreshed bytes and SHA rollover.
