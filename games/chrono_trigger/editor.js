...
const TABS=[{id:"text",label:"Text"},{id:"scenes",label:"Areas"},{id:"worlds",label:"Worlds"},{id:"worldnav",label:"World Exits"},{id:"animations",label:"Tiles"},{id:"exits",label:"Area Exits"},{id:"treasure",label:"Treasure"},{id:"palettes",label:"Palettes"}];
...
const state={tab:"text",source:"mine",busy:true,error:"",dashboard:null,dataMap:{rows:[]},changes:[],language:"en",textFiles:[],textPath:"",text:{rows:[]},paletteFiles:[],palettePath:"",palette:{rows:[]},scenes:{rows:[]},areaMode:"settings",sceneMapFiles:[],sceneMapPath:"",sceneMap:{rows:[]},worlds:{rows:[]},worldMode:"settings",worldFiles:[],worldFilePath:"",worldMap:{rows:[]},worldProps:{rows:[]},worldMusic:{rows:[]},worldColors:{rows:[]},worldNavigation:{rows:[]},animations:{rows:[]},graphicsSets:{rows:[]},assemblies:{rows:[]},tileMode:"animations",exits:{rows:[]},treasure:{rows:[]},dirty:new Map(),selected:{text:null,scenes:null,scenemaps:null,worlds:null,worldmaps:null,worldprops:null,worldmusic:null,worldcolors:null,worldnav:null,animations:null,tilesets:null,assemblies:null,exits:null,treasure:null,palettes:null},page:{text:0,scenes:0,scenemaps:0,worlds:0,worldmaps:0,worldprops:0,worldmusic:0,worldcolors:0,worldnav:0,animations:0,tilesets:0,assemblies:0,exits:0,treasure:0,palettes:0},pageSize:{text:18,scenes:18,scenemaps:18,worlds:18,worldmaps:18,worldprops:18,worldmusic:18,worldcolors:18,worldnav:18,animations:18,tilesets:18,assemblies:18,exits:18,treasure:18,palettes:18},query:{text:"",scenes:"",scenemaps:"",worlds:"",worldmaps:"",worldprops:"",worldmusic:"",worldcolors:"",worldnav:"",animations:"",tilesets:"",assemblies:"",exits:"",treasure:"",palettes:""},sort:{text:{key:"key",dir:1},scenes:{key:"id",dir:1},scenemaps:{key:"layer",dir:1},worlds:{key:"id",dir:1},worldmaps:{key:"layer",dir:1},worldprops:{key:"tileIndex",dir:1},worldmusic:{key:"index",dir:1},worldcolors:{key:"index",dir:1},worldnav:{key:"tableId",dir:1},animations:{key:"fileId",dir:1},tilesets:{key:"id",dir:1},assemblies:{key:"tileId",dir:1},exits:{key:"sceneId",dir:1},treasure:{key:"sceneId",dir:1},palettes:{key:"index",dir:1}},mapPage:0,mapQuery:"",mapStatus:"",mapSort:["filename",1],exportResult:null};
...
function sceneMapDetail(row){const low=row.layer===3?0:(row.upperBank?256:0),high=low+255;const bank=row.layer===3?"Layer 3 · 0-255":row.upperBank?"Upper · 256-511":"Lower · 0-255";return detailPanel({className:"ct-detail",title:`Layer ${row.layer} · ${row.xTile}, ${row.yTile}`,identity:recordId(row.mapId),meta:state.sceneMap.path||"",body:[
...
function areasSurface(){return state.areaMode==="map"?sceneMapsView():scenesView()}
function areaToolbar(){const mode=el("select",{"aria-label":"Area data",onchange:event=>switchAreaMode(event.target.value)},
  ...[["settings","Settings"],["map","Map tiles"]].map(([value,label])=>{const option=el("option",{value},label);option.selected=value===state.areaMode;return option}));
  const controls=[el("label",{},"Area data",mode)];
  if(state.areaMode==="map"){const file=el("select",{"aria-label":"Area map file",onchange:event=>switchSceneMapFile(event.target.value)},...state.sceneMapFiles.map(row=>{const option=el("option",{value:row.path},row.label);option.selected=row.path===state.sceneMapPath;return option}));controls.push(el("label",{},"File",file))}
  return controls}
...
function worldMapDetail(row){const low=row.layer===1?0:256,high=row.layer===1?255:511;return detailPanel({className:"ct-detail",title:`Layer ${row.layer} · ${row.xTile}, ${row.yTile}`,identity:recordId(row.mapId),meta:state.worldMap.path||"",body:[
  detailSection({title:"MAP TILE",body:[
...
function worldPropertyDetail(row){return detailPanel({className:"ct-detail",title:`Tile ${row.tileIndex}`,identity:recordId(row.fileId),meta:state.worldProps.path||"",body:[
...
function worldMusicDetail(row){return detailPanel({className:"ct-detail",title:`Music block ${row.xTile}, ${row.yTile}`,identity:recordId(row.fileId),meta:state.worldMusic.path||"",body:[
...
function worldColorDetail(row){const picker=el("input",{type:"color",class:"ct-color-input",value:row.hex.toLowerCase(),disabled:readonly(),oninput:event=>{remember("worldcolors",row,{hex:event.target.value.toUpperCase()});render()}});return detailPanel({className:"ct-detail",title:`Color ${row.index}`,identity:recordId(row.index),meta:state.worldColors.path||"",body:[
...
function graphicsSetDetail(row){const fields=Array.from({length:8},(_,index)=>detailField({label:`GRAPHICS SET ${index}`,...numberField("tilesets",row,`graphicsSet${index}`,0,255,index===6?"Animated-chip graphics set. 255 means unused.":"Graphics-set reference. 255 means this slot is unused.")}));return detailPanel({className:"ct-detail",title:`Tileset ${row.id}`,identity:recordId(row.id),meta:row.path,body:[detailSection({title:"GRAPHICS SETS",body:fields}),detailSection({title:"PRESERVED",body:[detailField({label:"TRAILING BYTES",control:readonlyField(String(row.trailingBytes))})]})]})}
...
function assemblyDetail(row){return detailPanel({className:"ct-detail",title:`Tile ${row.tileId} · ${row.cornerName}`,identity:recordId(row.fileId),meta:row.path,body:[
...
function tilesView(){if(state.tileMode==="tilesets")return graphicsSetsView();if(state.tileMode==="assemblies")return assembliesView();return animationsView()}
function tileToolbar(){const select=el("select",{"aria-label":"Tile data",onchange:event=>switchTileMode(event.target.value)},
  ...[["animations","Animations"],["tilesets","Graphics sets"],["assemblies","Assemblies"]].map(([value,label])=>{const option=el("option",{value},label);option.selected=value===state.tileMode;return option}));
