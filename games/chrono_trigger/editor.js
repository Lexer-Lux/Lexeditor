...
function records(tab,columns,detail,noun){const filtered=sorted(tab);if(!filtered.length)return el("div",{class:"lex-notice"},`No ${noun} match this view.`);if(!filtered.some(row=>row.token===state.selected[tab]))state.selected[tab]=filtered[0].token;return pagedListDetail({rows:filtered,key:row=>row.token,selected:state.selected[tab],page:state.page[tab],pageSize:state.pageSize[tab],noun,splitKey:`chrono-${tab}`,rowsKey:`chrono-${tab}`,defaultSplit:48,minLeft:320,minRight:380,search:{key:`chrono-${tab}`,value:state.query[tab],label:`Search ${noun}`,change:value=>{state.query[tab]=value;state.page[tab]=0;render()}},sync:next=>{state.page[tab]=next.page;state.pageSize[tab]=next.pageSize;if(next.selected!==null)state.selected[tab]=next.selected},change:next=>{state.page[tab]=next.page;state.pageSize[tab]=next.pageSize;if(next.selected!==null)state.selected[tab]=next.selected;render()},master:view=>master(tab,view,columns),detail})}
...
]}));return detailPanel({title:`Sprite ${row.id}`,identity:recordId(row.id),meta:row.path,body})}
