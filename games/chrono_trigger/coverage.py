...
def resource_override(path: str) -> dict | None:
    """Return classifications for structured resources integrated outside data.py."""
...
def augment_data_map(store: OverlayStore, payload: dict) -> dict:
    ...
    counts.update({"worldHeaders": 8 if has_bank else 0, "worldEventTables": world_tables})
