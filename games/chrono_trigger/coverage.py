...
def augment_data_map(store: OverlayStore, payload: dict) -> dict:
    ...
    for index, row in enumerate(rows):
        ...
        if filename.startswith("Game/world/esl/"):
            ...
            rows[index] = {
                **row,
                "controls": f"{field_scripts} field event scripts: PC disassembly/control flow plus named fixed-width item, comparison, script-memory, bit, PC-extended, call, object/property, movement/follow, location/party/screen, audio and other proven editors",
                "notes": "The desktop Events view and tools/chrono_trigger_event.py edit only proven fixed-width argument bytes through stale-hash-protected project overlays. Proven newer families include 0x12–0x15 script-memory comparisons; 0x63/64/69/6B/6F local bit operations; PC-only raw-slot 0x3A/3D/3E/45/46/70/74/78; 0x02–0x07 function calls with doubled target + priority/function nibbles; 0x0A/7C/7D doubled object controls; low-bit-only 0x0D/0x0E property editors that preserve unknown high bits; 0x8F/94–99/9A/A0/A1/B5/B6 movement/follow commands; D9/E2/E7/F4 party/location-from-memory/screen controls; EB song volume; and compact 29/82/C8 operands. Raw movement/party/scroll coordinates stay labeled as coordinate bytes unless units are independently evidenced. EC is now dynamically disassembled by its documented Sound-menu subcommand widths (1/2/3 argument bytes) but remains unwritable; unknown EC forms fail closed. Odd doubled targets, noncanonical shake values and ambiguous 0x16/60/61/67/6E/8D/8E/9E/9F encodings remain read-only. When a relative jump byte changes, its new target must be a decoded command boundary. Opcode changes, insertion/deletion, pointer relocation and variable/unresolved commands remain read-only.",
