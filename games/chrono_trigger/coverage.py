        ...
        if filename.startswith("Game/world/esl/"):
            ...
            rows[index] = {
                **row,
                ...
                "notes": "Desktop Events and tools/chrono_trigger_event.py edit only proven fixed-width argument bytes through stale-hash-protected project overlays. Current proven additions include 0x12–0x15 comparisons; narrow 0x1C bank result storage at 0x7F0000–0x7F00FF; bank assignments 0x53/54/56/58/59 with constructor-evidenced ranges; 0x63/64/69/6B/6F local bit ops; PC-only raw-slot 0x3A/3D/3E/45/46/70/74/78; 0x02–0x07 calls; doubled object controls 0x0A/0B/0C/7C/7D; doubled facing/result targets 0x23/24/A8/A9; directional NPC-facing 0x1E/1F/25/26 limited to the live 0x00–0x32 menu domain; 0x0D/0x0E known property bits with unknown-bit preservation; movement/follow 0x7A/8F/94–99/9A/9D/A0/A1/B5/B6; D9/E2/E7/F4 party/location/screen controls; EB song volume; and compact 29/82/C8 operands. Raw coordinates stay labeled as bytes unless units are independently evidenced. EC is dynamically disassembled by documented Sound-menu widths with read-only known-form semantics. Ambiguous 0x16/48–4D/60/61/65/66/67/6E/75/76/77/7B/27/28/8D/8E/92/9C/9E/9F and EC writes remain read-only. Changed jump bytes must target decoded command boundaries. Opcode changes, insertion/deletion, pointer relocation and variable/unresolved writes remain unsupported.",
