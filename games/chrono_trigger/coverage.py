...
def augment_data_map(store: OverlayStore, payload: dict) -> dict:
    ...
    for index, row in enumerate(rows):
        ...
        if filename.startswith("Game/world/esl/"):
            ...
            rows[index] = {
                **row,
                "controls": f"{field_scripts} field event scripts: PC disassembly/control flow, named fixed-width editors and read-only real-install coverage audit",
                "notes": "Desktop Events and tools/chrono_trigger_event.py edit only proven fixed-width argument bytes through stale-hash-protected project overlays. tools/chrono_trigger_event_audit.py can scan the real Steam archive plus project overlay (or immutable Vanilla), deduplicates aliased function bounds, excludes zero-argument commands from editor-gap metrics, prioritizes parser-stop opcodes and reports malformed overlays per event without aborting the full scan. Current proven additions include 0x12–0x16 comparisons, including canonical bank-7F page/operator packing for 0x16; narrow 0x1C bank result storage at 0x7F0000–0x7F00FF; bank assignments 0x53/54/56/58/59 with constructor-evidenced ranges; PC segment-memory 0x48–0x4D with raw u16 segment fields and raw local-slot/value operands only (no fabricated full RAM address); script-memory bit ops 0x63/64/69/6B/6F plus canonical bank-7F 0x65/66; PC-only raw-slot 0x3A/3D/3E/45/46/6E/70/74/78, with 0x6E exposed as raw extended slot + value + comparator + validated jump; 0x02–0x07 calls; doubled object controls 0x0A/0B/0C/7C/7D; doubled facing/result targets 0x23/24/A8/A9; directional NPC-facing 0x1E/1F/25/26 limited to the live 0x00–0x32 menu domain; 0x0D/0x0E known property bits with unknown-bit preservation; movement/follow 0x7A/8F/94–99/9A/9D/A0/A1/B5/B6; D9/E2/E7/F4 party/location/screen controls; EB song volume; and compact 29/82/C8 operands. Raw coordinates stay labeled as bytes unless units are independently evidenced. EC is dynamically disassembled by documented Sound-menu widths with read-only known-form semantics and is blocked from raw set-args writes too. PC parser contracts govern 0x2E/0x88/0x4E where generic menus differ. F1 stops fail-closed because public PC parser/table and live menu disagree on its one-vs-two-argument boundary. Ambiguous 0x60/61/67/75/76/77/7B/27/28/8D/8E/92/9C/9E/9F and dynamic/unresolved writes remain read-only. Changed jump bytes must target decoded command boundaries. Opcode changes, insertion/deletion and pointer relocation remain unsupported.",
