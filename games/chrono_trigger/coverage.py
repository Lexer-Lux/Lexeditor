        ...
        if filename.startswith("Game/world/esl/"):
            ...
            rows[index] = {
                **row,
                ...
                "notes": "The desktop Events view and tools/chrono_trigger_event.py edit only proven fixed-width argument bytes through stale-hash-protected project overlays. Proven newer families include 0x12–0x15 script-memory comparisons; narrow 0x1C result storage at encodable 0x7F0000–0x7F00FF; 0x63/64/69/6B/6F local bit operations; PC-only raw-slot 0x3A/3D/3E/45/46/70/74/78; 0x02–0x07 function calls; doubled object controls 0x0A/0B/0C/7C/7D; doubled facing targets 0x23/24/A8/A9; low-bit-only 0x0D/0x0E property editors that preserve unknown high bits; 0x8F/94–99/9A/9D/A0/A1/B5/B6 movement/follow commands; D9/E2/E7/F4 party/location-from-memory/screen controls; EB song volume; and compact 29/82/C8 operands. Raw movement/party/scroll coordinates stay labeled as coordinate bytes unless units are independently evidenced. EC is dynamically disassembled by documented Sound-menu subcommand widths and receives read-only known-form semantics, but remains unwritable. Odd doubled targets, noncanonical shake values and ambiguous 0x16/60/61/67/6E/27/28/8D/8E/92/9C/9E/9F encodings remain read-only. When a relative jump byte changes, its new target must be a decoded command boundary. Opcode changes, insertion/deletion, pointer relocation and variable/unresolved command writes remain read-only.",
