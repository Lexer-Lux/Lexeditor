...
def augment_data_map(store: OverlayStore, payload: dict) -> dict:
    ...
    for index, row in enumerate(rows):
        ...
        if filename.startswith("Game/world/esl/"):
            ...
            rows[index] = {
                **row,
                "controls": f"{field_scripts} field event scripts: PC disassembly/control flow plus named fixed-width item, comparison, script/bank/segment memory, bit, PC-extended, call, object/facing/property, movement/follow, location/party/screen, audio and other proven editors",
