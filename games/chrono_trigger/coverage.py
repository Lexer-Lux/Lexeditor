...
def augment_data_map(store: OverlayStore, payload: dict) -> dict:
    ...
    for index, row in enumerate(rows):
        ...
        if filename.startswith("Game/world/esl/"):
            ...
            rows[index] = {
                **row,
                "controls": f"{field_scripts} field event scripts: PC disassembly/control flow plus named fixed-width item, comparison, script-memory, bit, call, object, movement/follow, party/screen and other proven editors",
                "notes": "The desktop Events view and tools/chrono_trigger_event.py edit only proven fixed-width argument bytes through stale-hash-protected project overlays. Newer proven families include 0x12–0x15 script-memory comparisons, 0x63/64/69/6B/6F local bit operations, 0x02–0x07 function calls with doubled target + priority/function nibbles, 0x0A/7C/7D doubled object controls, 0x8F/94–99/9A/A0/A1/B5/B6 movement/follow commands, and D9/E7/F4 party/screen controls. Raw movement/party/scroll coordinates stay labeled as coordinate bytes unless units are independently evidenced. Odd doubled targets, noncanonical shake values and ambiguous 0x16/67/6E/8D/8E/9E/9F encodings remain read-only. When a relative jump byte changes, its new target must be a decoded command boundary. Opcode changes, insertion/deletion, pointer relocation and variable/unresolved commands remain read-only.",
