"""Conservative structured editor for current Build 42 sound scalars.

Sound clips are nested child blocks with their own file/distance/volume semantics.
Lexeditor edits only existing typed top-level sound properties and preserves clips
and unknown data verbatim.
"""
...
EDITABLE_FIELDS = ("category", "is3D", "loop", "master", "maxInstancesPerEmitter")
BOOLEAN_FIELDS = {"is3D", "loop"}
MASTER_VALUES = ("Primary", "Ambient", "Music", "VehicleEngine")


def _blocks(text: str) -> list[tuple[core.Block, core.Block]]:
    masked = core._masked_code(text)
    pattern = re.compile(r"\bsound\s+([^\s{]+(?:\s+[^\s{]+)*)\s*\{", re.IGNORECASE)
    ...
    for module in core._top_level_blocks(text, "module"):
        ...
        while cursor < module.close_brace:
            ...
            rows.append((module, core.Block("sound", match.group(1).strip(), match.start(), opened, closed)))
            cursor = closed + 1
            body_start = cursor
    return rows
    ...
    for path in core.script_paths(root):
        ...
        for module, block in pairs:
            ...
            rows.append({
                ...
                "masterOptions": list(MASTER_VALUES),
                "hasClips": bool(re.search(r"\bclip\s*\{", core._masked_code(text[block.open_brace + 1:block.close_brace]), re.IGNORECASE)),
            })
...
def _validate(key: str, value: object) -> str:
    ...
    if key in BOOLEAN_FIELDS:
        ...
        return lowered
    if key == "master":
        if clean not in MASTER_VALUES:
            raise core.ProjectZomboidError("master is not a documented Build 42 sound master")
        return clean
    if key == "maxInstancesPerEmitter":
        try:
            return str(int(clean))
        except ValueError as error:
            raise core.ProjectZomboidError("maxInstancesPerEmitter must be an integer") from error
    return clean


def save(root: Path, relative: str, module_name: str, sound_id: str,
    ...
    if not isinstance(edits, dict) or not edits or set(edits) - set(EDITABLE_FIELDS):
        raise core.ProjectZomboidError("Save contains unsupported sound fields")

    matches = [(module, block) for module, block in _blocks(text)
               if module.name == module_name and block.name == sound_id]
    if len(matches) != 1:
        raise core.ProjectZomboidError("Sound identity is missing or ambiguous")
    _module, block = matches[0]
    values, duplicates = core._properties(text, block)
    unsafe = set(duplicates) & set(edits)
    if unsafe:
        raise core.ProjectZomboidError("Cannot safely edit duplicated sound properties: " + ", ".join(sorted(unsafe)))
    ...
    if len(replacements) != len(validated):
        raise core.ProjectZomboidError("Could not locate every sound property safely")
    ...
    return next(row for row in read(root)["rows"]
                if row["path"] == relative and row["module"] == module_name and row["id"] == sound_id)
