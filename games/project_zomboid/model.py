"""Conservative structured editor for current Build 42 module-level model scalars.

Models can also appear under vehicles and parts, and can contain nested attachment
blocks. Lexeditor edits only existing explicitly typed module-level scalar fields and
preserves nested model structure verbatim.
"""
...
BOOLEAN_FIELDS = {"invertX", "static", "undoCoreScale"}
CULL_FACE_VALUES = ("Back", "Front", "None")


def _blocks(text: str) -> list[tuple[core.Block, core.Block]]:
    masked = core._masked_code(text)
    pattern = re.compile(r"\bmodel\s+([^\s{]+(?:\s+[^\s{]+)*)\s*\{", re.IGNORECASE)
    ...
    for module in core._top_level_blocks(text, "module"):
        ...
        while cursor < module.close_brace:
            ...
            rows.append((module, core.Block("model", match.group(1).strip(), match.start(), opened, closed)))
            cursor = closed + 1
            body_start = cursor
    return rows
    ...
    for path in core.script_paths(root):
        ...
        for module, block in pairs:
            ...
            rows.append({
                ...
                "cullFaceOptions": list(CULL_FACE_VALUES),
                "hasAttachments": bool(re.search(
                    r"\battachment\s+[^\s{]+(?:\s+[^\s{]+)*\s*\{",
                    core._masked_code(text[block.open_brace + 1:block.close_brace]),
                    re.IGNORECASE,
                )),
            })
...
def _validate(key: str, value: object) -> str:
    ...
    if key in BOOLEAN_FIELDS:
        ...
        return lowered
    if key == "cullFace":
        if clean not in CULL_FACE_VALUES:
            raise core.ProjectZomboidError("cullFace is not a documented Build 42 culling mode")
        return clean
    if key == "scale":
        try:
            number = float(clean)
...
def save(root: Path, relative: str, module_name: str, model_id: str,
    ...
    if not isinstance(edits, dict) or not edits or set(edits) - set(EDITABLE_FIELDS):
        raise core.ProjectZomboidError("Save contains unsupported model fields")

    matches = [(module, block) for module, block in _blocks(text)
               if module.name == module_name and block.name == model_id]
    if len(matches) != 1:
        raise core.ProjectZomboidError("Model identity is missing or ambiguous")
    _module, block = matches[0]
    values, duplicates = core._properties(text, block)
    unsafe = set(duplicates) & set(edits)
    if unsafe:
        raise core.ProjectZomboidError("Cannot safely edit duplicated model properties: " + ", ".join(sorted(unsafe)))
    ...
    if len(replacements) != len(validated):
        raise core.ProjectZomboidError("Could not locate every model property safely")
    ...
    return next(row for row in read(root)["rows"]
                if row["path"] == relative and row["module"] == module_name and row["id"] == model_id)
