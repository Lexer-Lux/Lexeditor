"""Conservative Build 42 fixing editor.

The current fixing schema has one independently typed primitive suitable for
surgical editing: ConditionModifier. Repair ingredients and fixer grammar remain
read-only and are preserved verbatim.
"""
...
EDITABLE_FIELDS = ("ConditionModifier",)


def _blocks(text: str) -> list[tuple[core.Block, core.Block]]:
    masked = core._masked_code(text)
    pattern = re.compile(r"\bfixing\s+([^\s{]+(?:\s+[^\s{]+)*)\s*\{", re.IGNORECASE)
    ...
    for module in core._top_level_blocks(text, "module"):
        ...
        while cursor < module.close_brace:
            ...
            result.append((module, core.Block(
                "fixing", match.group(1).strip(), match.start(), opened, closed
            )))
            cursor = closed + 1
            body_start = cursor
    return result
...
def _validate(value: object) -> str:
    clean = core._clean_scalar(value, "ConditionModifier", allow_empty=False)
    if any(char in clean for char in ",{}"):
        raise core.ProjectZomboidError("ConditionModifier contains script punctuation")
    try:
        number = float(clean)
    except ValueError as error:
        raise core.ProjectZomboidError("ConditionModifier must be a number") from error
    if not math.isfinite(number):
        raise core.ProjectZomboidError("ConditionModifier must be finite")
    return clean


def save(root: Path, relative: str, module_name: str, fixing_id: str,
    ...
    if not isinstance(edits, dict) or not edits or set(edits) - set(EDITABLE_FIELDS):
        raise core.ProjectZomboidError("Save contains unsupported fixing fields")

    matches = [(module, block) for module, block in _blocks(text)
               if module.name == module_name and block.name == fixing_id]
    if len(matches) != 1:
        raise core.ProjectZomboidError("Fixing identity is missing or ambiguous")
    _module, block = matches[0]
    values, duplicates = core._properties(text, block)
    unsafe = set(duplicates) & set(edits)
    if unsafe:
        raise core.ProjectZomboidError(
            "Cannot safely edit duplicated fixing properties: " + ", ".join(sorted(unsafe))
        )
    ...
    validated = {"ConditionModifier": _validate(edits["ConditionModifier"])}
    ...
    for match in core._PROPERTY_RE.finditer(body):
        for char in masked[last:match.start()]:
            if char == "{":
                ...
                depth = max(0, depth - 1)
        last = match.end()
        if depth == 0 and match.group("key") == "ConditionModifier":
            replacements.append((
                body_start + match.start("value"),
                body_start + match.end("value"),
                validated["ConditionModifier"],
            ))
    if len(replacements) != 1:
        raise core.ProjectZomboidError("Could not locate ConditionModifier safely")
    ...
    return next(row for row in read(root)["rows"]
                if row["path"] == relative and row["module"] == module_name and row["id"] == fixing_id)
