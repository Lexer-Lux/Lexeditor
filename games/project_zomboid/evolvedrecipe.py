"""Structured Build 42 evolvedrecipe reader/writer.

Field semantics are limited to the current PZ Wiki Modding evolvedrecipe schema.
Writes patch existing top-level scalar properties only and preserve everything else.
"""
...
BOOLEAN_FIELDS = {"AddIngredientIfCooked", "CanAddSpicesEmpty"}
ITEM_REFERENCE_FIELDS = {"BaseItem", "ResultItem"}


def _module_blocks(text: str) -> list[tuple[core.Block, core.Block]]:
    masked = core._masked_code(text)
    pattern = re.compile(r"\bevolvedrecipe\s+([^\s{]+(?:\s+[^\s{]+)*)\s*\{")
    ...
    for module in core._top_level_blocks(text, "module"):
        ...
        while cursor < module.close_brace:
            ...
            found.append((module, core.Block("evolvedrecipe", match.group(1).strip(), match.start(), open_brace, close_brace)))
...
def _validate(key: str, value: object) -> str:
    ...
    if key == "Cookable":
        if clean.casefold() != "true":
            raise core.ProjectZomboidError("Cookable is presence-only in Build 42 and may only be true")
        return "true"
    if key == "MaxItems":
        try:
            number = int(clean)
        except ValueError as error:
            raise core.ProjectZomboidError("MaxItems must be an integer") from error
        if number < 1:
            raise core.ProjectZomboidError("MaxItems must be >= 1")
        return str(number)
    if key == "MinimumWater":
        try:
            number = float(clean)
        except ValueError as error:
            raise core.ProjectZomboidError("MinimumWater must be a number") from error
        if not math.isfinite(number):
            raise core.ProjectZomboidError("MinimumWater must be finite")
        return clean
    if key in ITEM_REFERENCE_FIELDS and clean and "." not in clean:
        raise core.ProjectZomboidError(f"{key} must use a full item type such as Base.Item")
    return clean
...
def save(root: Path, relative: str, module_name: str, recipe_id: str,
    ...
    allowed = {__import__('os').path.normcase(str(candidate.resolve())) for candidate in core.script_paths(root)}
    if __import__('os').path.normcase(str(path.resolve())) not in allowed:
    ...
    if not isinstance(edits, dict) or not edits or set(edits) - set(EDITABLE_FIELDS):
        raise core.ProjectZomboidError("Save contains unsupported evolvedrecipe fields")

    matches = [(module, block) for module, block in _module_blocks(text)
               if module.name == module_name and block.name == recipe_id]
    if len(matches) != 1:
        raise core.ProjectZomboidError("Evolved recipe identity is missing or ambiguous")
    _module, block = matches[0]
    values, duplicates = core._properties(text, block)
    unsafe = set(duplicates) & set(edits)
    if unsafe:
        raise core.ProjectZomboidError("Cannot safely edit duplicated evolvedrecipe properties: " + ", ".join(sorted(unsafe)))
    ...
    for match in core._PROPERTY_RE.finditer(body):
        ...
        if depth == 0 and key in validated:
            ...
            replacements.append((start, end, validated[key]))
    if {text[start:end].strip() and key for key in () for start,end,_ in replacements}:  # pragma: no cover
        pass
    if len(replacements) != len(validated):
        raise core.ProjectZomboidError("Could not locate every evolvedrecipe property safely")
    output = text
    for start, end, value in sorted(replacements, reverse=True):
        output = output[:start] + value + output[end:]
    core._atomic_write(path, output.encode("utf-8"))
    rows = read(root)["rows"]
    return next(row for row in rows if row["path"] == relative and row["module"] == module_name and row["id"] == recipe_id)
