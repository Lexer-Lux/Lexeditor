"""Conservative structured editor for current Build 42 mannequin scalars.

Mannequins reference model blocks. Lexeditor keeps that reference and all unknown
future data read-only, editing only existing scalar properties whose current schema
is explicit.
"""
...
EDITABLE_FIELDS = ("animSet", "animState", "female", "outfit", "pose", "texture")
STRING_FIELDS = {"animSet", "animState", "outfit", "pose", "texture"}


def _blocks(text: str) -> list[tuple[core.Block, core.Block]]:
    masked = core._masked_code(text)
    pattern = re.compile(r"\bmannequin\s+([^\s{]+(?:\s+[^\s{]+)*)\s*\{", re.IGNORECASE)
    ...
    for module in core._top_level_blocks(text, "module"):
        ...
        while cursor < module.close_brace:
            ...
            rows.append((module, core.Block("mannequin", match.group(1).strip(), match.start(), opened, closed)))
            cursor = closed + 1
            body_start = cursor
    return rows
    ...
    for path in core.script_paths(root):
        ...
        for module, block in pairs:
            ...
            rows.append({
                ...
                "modelReference": values.get("model", ""),
            })
    return {"rows": rows, "errors": errors}


def _validate(key: str, value: object) -> str:
    allow_empty = key == "outfit"
    clean = core._clean_scalar(value, key, allow_empty=allow_empty)
    if any(char in clean for char in ",{}"):
        raise core.ProjectZomboidError(f"{key} contains script punctuation")
    if key == "female":
        lowered = clean.casefold()
        if lowered not in {"true", "false"}:
            raise core.ProjectZomboidError("female must be true or false")
        return lowered
    if key in STRING_FIELDS:
        return clean
    raise core.ProjectZomboidError(f"Unsupported mannequin field: {key}")


def save(root: Path, relative: str, module_name: str, mannequin_id: str,
    ...
    if not isinstance(edits, dict) or not edits or set(edits) - set(EDITABLE_FIELDS):
        raise core.ProjectZomboidError("Save contains unsupported mannequin fields")

    matches = [(module, block) for module, block in _blocks(text)
               if module.name == module_name and block.name == mannequin_id]
    if len(matches) != 1:
        raise core.ProjectZomboidError("Mannequin identity is missing or ambiguous")
    _module, block = matches[0]
    values, duplicates = core._properties(text, block)
    unsafe = set(duplicates) & set(edits)
    if unsafe:
        raise core.ProjectZomboidError("Cannot safely edit duplicated mannequin properties: " + ", ".join(sorted(unsafe)))
    ...
    if len(replacements) != len(validated):
        raise core.ProjectZomboidError("Could not locate every mannequin property safely")
    ...
    return next(row for row in read(root)["rows"]
                if row["path"] == relative and row["module"] == module_name and row["id"] == mannequin_id)
