...
def smoke() -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="lexeditor-project-zomboid-") as temp_name:
        ...
        script.write_text(
            "module LexSmoke\n{\n"
            " item TestItem { DisplayCategory = Tool, ItemType = base:normal, Weight = 0.3, Icon = Radio, UnknownFutureField = KeepMe, }\n"
            " evolvedrecipe TestSoup { BaseItem = Base.PotOfSoup, ResultItem = Base.PotOfSoup, MaxItems = 4, CanAddSpicesEmpty = true, MinimumWater = 0.0, }\n"
            " craftRecipe MakeTestThing { AllowBatchCraft = true, CanWalk = false, category = General, Icon = Radio, ResearchSkillLevel = -1, tags = InHandCraft, time = 50, timedAction = Craft, inputs { item 1 [Base.Plank], } }\n"
            " fluid TestFluid { ColorReference = Azure, DisplayName = Fluid_Name_TestFluid, Properties { HungerChange = -5, } }\n"
            " vehicle TestCar { engineForce = 3000, engineIdleSpeed = 750, engineLoudness = 100, engineQuality = 100, engineRepairLevel = 4, engineRPMType = jeep, gearRatioCount = 5, hasLighter = true, isSmallVehicle = false, part Engine { category = engine, } }\n"
            "}\n", encoding="utf-8")
...
PLUGIN = GamePlugin(
    ...
    installation=GameInstallSpec(
        root_env="LEXEDITOR_PROJECT_ZOMBOID_ROOT", required_paths=("ProjectZomboid64.exe", "media/scripts"),
