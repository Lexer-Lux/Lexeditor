---
name: add-game
description: Starting, scoping or filing issues for a Lexeditor game plugin. Use before writing a new plugin or parser, and when creating or checking its five standard issues.
---
...
label (the plugin id). Do not create a `Plugin` parent. An edition that runs another plugin's code shares that
plugin's label and issues through `issueLabel` in its `plugin.json` (ff7-2013
files under `ff7`). Track these five standard issues in this order,
...
2. `UX Refinement` (blocked by `Create Editor`): Starts `waiting`, as Lexer
   explicitly requested. End with an unchecked action for Lexer to identify
   the next screen or interaction to refine, or report prepared acceptance
   results. This specific scheduling exception does not claim unfinished work
   is complete. Make the screens usable and
...
issue carries its own workflow label (see `.agents/skills/github-issues/SKILL.md`).
Retire obsolete Plugin containers by detaching their children and closing the
container as not planned; preserve every child and all discussion.
Source, rendered UI, mod compatibility, delivered candidate and in-game
