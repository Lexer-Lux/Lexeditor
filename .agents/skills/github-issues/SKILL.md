...
- Game plugins' five independent standard issues: see
  `.agents/skills/add-game/SKILL.md`.

## Workflow labels

Every open issue has exactly one of these. Game, bug, enhancement and priority
labels are separate from status. Do not create Plugin parent issues.
...
scheduling note, not automatically `waiting`. Lexer's explicit exception is
the standard UX Refinement trackers, which start and remain waiting for his
next UX feedback or prepared acceptance results (see the add-game skill).
