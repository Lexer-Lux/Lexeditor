...
def _module_blocks(text: str) -> list[tuple[core.Block, core.Block]]:
    ...
    for module in core._top_level_blocks(text, "module"):
        ...
        while cursor < module.close_brace:
            ...
            found.append((
                module,
                core.Block("evolvedrecipe", match.group(1).strip(), match.start(), open_brace, close_brace),
            ))
...
def _validate(key: str, value: object) -> str:
    ...
    if key == "Cookable":
        if clean.casefold() != "true":
            raise core.ProjectZomboidError(
                "Cookable is presence-only in Build 42 and may only be true"
            )
        return "true"
    if key == "MaxItems":
        try:
    ...
    if key in ITEM_REFERENCE_FIELDS and clean and "." not in clean:
        raise core.ProjectZomboidError(
            f"{key} must use a full item type such as Base.Item"
        )
    return clean
...
def save(root: Path, relative: str, module_name: str, recipe_id: str,
    ...
    allowed = {
        ...
        for candidate in core.script_paths(root)
    }
    if os.path.normcase(str(path.resolve())) not in allowed:
        raise core.ProjectZomboidError(
            "Script is outside supported Build 42 script roots"
        )
    data, text = core._read_utf8(path)
    if core.sha256_bytes(data) != expected_sha256:
        raise core.ProjectZomboidError(
            "Script changed outside Lexeditor; reload before saving"
        )
    if not isinstance(edits, dict) or not edits or set(edits) - set(EDITABLE_FIELDS):
        raise core.ProjectZomboidError(
            "Save contains unsupported evolvedrecipe fields"
        )

    matches = [
        (module, block)
        for module, block in _module_blocks(text)
        if module.name == module_name and block.name == recipe_id
    ]
    if len(matches) != 1:
        raise core.ProjectZomboidError(
            "Evolved recipe identity is missing or ambiguous"
        )
    ...
    if unsafe:
        raise core.ProjectZomboidError(
            "Cannot safely edit duplicated evolvedrecipe properties: "
            + ", ".join(sorted(unsafe))
        )
    missing = set(edits) - set(values)
    if missing:
        raise core.ProjectZomboidError(
            "Writer changes existing properties only; missing: "
            + ", ".join(sorted(missing))
        )
    ...
    if len(replacements) != len(validated):
        raise core.ProjectZomboidError(
            "Could not locate every evolvedrecipe property safely"
        )
