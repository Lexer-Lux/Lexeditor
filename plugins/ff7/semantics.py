...
INITIAL_CONDITIONS = (
    ...
    (0x08, "Targetable at battle start"),
...
SCENE = {
    ...
    "encounters": {
        ...
        **{f"slot{i}_row": _field(label=f"Enemy slot {i+1} depth row", group=f"Enemy slot {i+1}", help="Formation depth/order, not a simple front/back toggle. Lower-numbered rows stand in front of higher-numbered rows; shared cover groups can block short-range targeting of enemies farther back.") for i in range(6)},
        "flags": _field(label="Battle restrictions", dataType="flags", flags=flags(*BATTLE_FLAGS), invertBits=True, bitWidth=16, group="Battle setup", help="Logical formation restrictions. Turning these restrictions on prevents escape, victory poses, or a preemptive strike. Unknown bits are preserved."),
        ...
        "cameraIndex": advanced("Pre-battle camera", "Opening camera sequence selected for this battle. This index is separate from the three stored camera placements; named sequences are not yet mapped."),
    },
}
...
for slot in range(6):
    ...
    SCENE["encounters"][f"slot{slot}_flags"] = _field(label=f"Enemy slot {slot+1} starting conditions", dataType="flags", flags=flags(*INITIAL_CONDITIONS), bitWidth=32, group=f"Enemy slot {slot+1}", help="Known scene.bin starting-state bits: visibility, side, targetability and whether the main AI script starts active. Unknown bits are preserved.")
