...
def data_map_rows() -> dict:
    rows = [
        ...
        {"filename": "battle/c0m*.dat", "controls": "Battle model section inventory, geometry counts and texture previews; enemy stats, properties, action definitions, structural and source-language conditional battle AI, and local scripted battle text", "notes": "The Enemies AI view decodes and rebuilds the five proved Init, Turn, Counter, Death and Pre-hit scripts. Structure supports replace, insert, delete and reorder; Source supports stable labels, exact opcode mnemonics and typed operands. Both compile through the same validated branch-aware writer. The Battle Text view edits the existing local dialogue lines referenced by those scripts. Unknown or malformed tails remain fail-closed. The Models tab shows the section inventory, geometry counts, and texture previews for every monster, character, and weapon model with whole-file replacement; stages, magic effects, and other undecoded battle files stay visible but locked.", "status": "partial"},
        ...
        {"filename": "Data/Sound/audio.dat + audio.fmt", "controls": "All 2,791 sound effects with format, duration, battle usage, previews, and per-sound replacement", "notes": "The SFX tab plays every valid entry, names proven battle-actor usage, and attributes mod sfx/ files to the sounds they replace using FFNx's verified external names. Placeholder entries stay visible but locked. Replacements load in-game only while FFNx external SFX are enabled.", "status": "partial"},
        {"filename": "textures/ (FFNx external texture overrides)", "controls": "World and battle-model texture previews with mod-file attribution", "notes": "The Textures tab previews the 20 world texl textures and every battle-model TIM, and shows which mod files replace each one using only FFNx base paths proved by its source. Files it cannot attribute stay visible as unmapped mod textures. World-texture replacement stays under Maps > World and model replacement under Models; this tab grows no second editor.", "status": "partial"},
