  ...
  async function loadDataset(dataset,tolerant=false){const suffix=`dataset=${encodeURIComponent(dataset)}`,get=path=>tolerant?api(path).catch(()=>null):api(path);const [battleItems,ammoEffects,cards,items,menuItems,shops,weapons,magic,gfs,characters,abilityJunction,abilityCommand,abilityStat,abilityCharacter,abilityParty,abilityGf,abilityMenu,text,names,enemies,enemyTables,enemyAi,enemyBattleText,refine,encounters,world,fields,init,sfx,models,textures]=await Promise.all([get(`/api/kernel?section=8&${suffix}`),get(`/api/kernel?section=22&${suffix}`),get(`/api/cards?${suffix}`),get(`/api/items?${suffix}`),get(`/api/menu-items?${suffix}`),get(`/api/shops?${suffix}`),get(`/api/weapons?${suffix}`),get(`/api/kernel?section=2&${suffix}`),get(`/api/kernel?section=3&${suffix}`),get(`/api/kernel?section=7&${suffix}`),get(`/api/kernel?section=12&${suffix}`),get(`/api/kernel?section=13&${suffix}`),get(`/api/kernel?section=14&${suffix}`),get(`/api/kernel?section=15&${suffix}`),get(`/api/kernel?section=16&${suffix}`),get(`/api/kernel?section=17&${suffix}`),get(`/api/kernel?section=18&${suffix}`),get(`/api/text?${suffix}`),get(`/api/namedic?${suffix}`),get(`/api/enemies?${suffix}`),get(`/api/enemy-tables?${suffix}`),get(`/api/enemy-ai?${suffix}`),get(`/api/enemy-battle-text?${suffix}`),get(`/api/refine?${suffix}`),get(`/api/encounters?${suffix}`),get(`/api/world-map?${suffix}`),get(`/api/fields?${suffix}`),get(`/api/init?${suffix}`),get(`/api/sfx?${suffix}`),get(`/api/models?${suffix}`),get(`/api/textures?${suffix}`)]);return {battleItems,ammoEffects,cards,items,menuItems,shops,weapons,magic,gfs,characters,abilityJunction,abilityCommand,abilityStat,abilityCharacter,abilityParty,abilityGf,abilityMenu,text,names,enemies,enemyTables,enemyAi,enemyBattleText,refine,encounters,world,fields,init,sfx,models,textures}}
