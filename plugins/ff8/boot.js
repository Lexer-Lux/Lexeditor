  ...
  async function saveAll(){
    try{
      ...
      // namedic.bin is a file of its own, and its writer rebuilds the whole
      // offset table, so only the names that changed are sent.
      if(signature(state.data.names.rows)!==signature(state.base.names)){
        const edits=state.data.names.rows.filter((row,index)=>row.text!==state.base.names[index].text)
          .map(row=>({index:row.id,text:row.text}));
        jobs.push(api("/api/namedic/save",post({edits})));
      }
      // The world-to-field table is a separate file too, and its writer
      // keeps every byte it does not name, so only changed rows are sent.
      if(signature(state.data.wm2field.rows)!==signature(state.base.wm2field)){
        const edits=state.data.wm2field.rows.filter((row,index)=>{
          const before=state.base.wm2field[index];
          return row.x!==before.x||row.y!==before.y||row.z!==before.z||row.fieldId!==before.fieldId;})
          .map(row=>({id:row.id,x:row.x,y:row.y,z:row.z,fieldId:row.fieldId}));
        jobs.push(api("/api/wm2field/save",post({edits})));
      }
  ...
  async function loadDataset(dataset,tolerant=false){const suffix=`dataset=${encodeURIComponent(dataset)}`,get=path=>tolerant?api(path).catch(()=>null):api(path);const [battleItems,ammoEffects,cards,items,menuItems,shops,weapons,magic,gfs,characters,abilityJunction,abilityCommand,abilityStat,abilityCharacter,abilityParty,abilityGf,abilityMenu,text,names,wm2field,enemies,enemyTables,enemyAi,enemyBattleText,refine,encounters,world,fields,init,sfx,models,textures]=await Promise.all([get(`/api/kernel?section=8&${suffix}`),get(`/api/kernel?section=22&${suffix}`),get(`/api/cards?${suffix}`),get(`/api/items?${suffix}`),get(`/api/menu-items?${suffix}`),get(`/api/shops?${suffix}`),get(`/api/weapons?${suffix}`),get(`/api/kernel?section=2&${suffix}`),get(`/api/kernel?section=3&${suffix}`),get(`/api/kernel?section=7&${suffix}`),get(`/api/kernel?section=12&${suffix}`),get(`/api/kernel?section=13&${suffix}`),get(`/api/kernel?section=14&${suffix}`),get(`/api/kernel?section=15&${suffix}`),get(`/api/kernel?section=16&${suffix}`),get(`/api/kernel?section=17&${suffix}`),get(`/api/kernel?section=18&${suffix}`),get(`/api/text?${suffix}`),get(`/api/namedic?${suffix}`),get(`/api/wm2field?${suffix}`),get(`/api/enemies?${suffix}`),get(`/api/enemy-tables?${suffix}`),get(`/api/enemy-ai?${suffix}`),get(`/api/enemy-battle-text?${suffix}`),get(`/api/refine?${suffix}`),get(`/api/encounters?${suffix}`),get(`/api/world-map?${suffix}`),get(`/api/fields?${suffix}`),get(`/api/init?${suffix}`),get(`/api/sfx?${suffix}`),get(`/api/models?${suffix}`),get(`/api/textures?${suffix}`)]);return {battleItems,ammoEffects,cards,items,menuItems,shops,weapons,magic,gfs,characters,abilityJunction,abilityCommand,abilityStat,abilityCharacter,abilityParty,abilityGf,abilityMenu,text,names,wm2field,enemies,enemyTables,enemyAi,enemyBattleText,refine,encounters,world,fields,init,sfx,models,textures}}
  ...
  async function navigate(tab){if(!(await enemyAiBeforeLeave()))return false;if(tab==="formulae"){state.settingsTab="formulae";tab="settings"}if(/^ability[A-Z]/.test(tab)){state.abilityTab=tab;tab="abilities"}if(tab==="names"){state.textTab="names";tab="text"}state.tab=tab;render()}
