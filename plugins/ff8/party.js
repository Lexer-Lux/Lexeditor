  ...
  function gfAbilities(fields,row){const groups=new Map();for(const field of fields){const key=field.row||field.field;if(!groups.has(key))groups.set(key,[]);groups.get(key).push(field)}const find=(values,suffix="")=>values.find(field=>suffix?field.field.endsWith(suffix):/^ability\d+$/.test(field.field)),rows=[...groups].map(([key,values])=>({key,slot:Number(String(key).replace(/^ability/,""))||0,ability:find(values),level:find(values,"_level_or_prereq"),alternate:find(values,"_alt_prereq")})),[sortKey,sortDir]=state.gfSorts.abilities,sorted=rows.sort((a,b)=>sortDir*(sortKey==="slot"?a.slot-b.slot:String(displayFieldValue(a[sortKey])).localeCompare(String(displayFieldValue(b[sortKey])),undefined,{numeric:true,sensitivity:"base"})));sorted.forEach(entry=>{entry.id=Number(String(entry.key).replace(/^ability/,""))||0});const slotNames=sorted.map(entry=>[entry.id,entry.ability?.lookup?.entries?.find(o=>Number(o.value)===Number(entry.ability.value))?.name||`slot ${entry.id}`]).sort((a,b)=>a[0]-b[0]);const table=columnList({rows:sorted,key:entry=>entry.key,editable:true,fill:true,sortState:{key:sortKey,dir:sortDir},sort:key=>sortGfTable("abilities",key),template:"52px minmax(125px,1fr) minmax(235px,1.5fr) minmax(76px,.5fr)",columns:[{key:"slot",label:"Slot",sortable:true,sortValue:entry=>entry.slot,numberedId:true,render:entry=>entry.id},{key:"ability",label:"Ability",sortable:true,render:entry=>entry.ability?fieldSourceControl(entry.ability,"gfs",row.id):"—"},{key:"level",label:"Prereq",help:"Choose whether this ability unlocks at a GF level or after another ability slot is learned. Values 1–100 are a GF level; 101–121 are the matching ability slot on this GF.",sortable:true,render:entry=>entry.level?LexeditorUI.controlGroup([gfPrereqKind(entry.level,row),Number(entry.level.value)>100?gfPrereqSlot(entry.level,slotNames):fieldSourceControl(entry.level,"gfs",row.id)],{className:"gf-prereq-cell"}):"—"},{key:"alternate",label:"Alt. Prereq",help:"Points to another ability slot on this GF. This ability stays available only while that other ability is unfinished. 255 means no alternate restriction.",sortable:true,render:entry=>entry.alternate?fieldSourceControl(entry.alternate,"gfs",row.id):"—"}]});table.dataset.gfAbilities="true";return detailSection({title:"ABILITIES",
