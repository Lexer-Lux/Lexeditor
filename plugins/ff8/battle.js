  ...
  function worldDrawPointDetail(row,prefs){
    const position=worldDrawPosition(row),map=LexeditorUI.imageMap({fill:false,label:`Set Draw Point ${row.drawId} position`,columns:8,rows:8,ratio:4/3,
      points:[{x:position.x/128,y:position.y/96,label:`Draw Point ${row.drawId}`}],
    ...
    return sharedDetail({...row,name:`DRAW POINT ${row.drawId}`},prefs,[detailSection({className:"world-draw-position",title:"WORLD POSITION",help:infoHelp("Section 34 stores only this world Draw Point's X, Y, and sub-ID bytes. Its magic, quantity, and refill behavior live in FF8_EN.exe and are not invented here."),body:[map,el("p",{class:"world-draw-help"},"Click the grid to place the point, or enter exact byte coordinates."),detailField({label:"X",help:infoHelp("Packed position byte: its lower seven bits give the block column; its top bit gives the odd or even block row. Use the grid for visual placement."),control:worldNumber(row,"x",0,255,`Draw Point ${row.drawId} X`)}),detailField({label:"Y",help:infoHelp("Packed row byte. The block row is twice this value plus the top bit of X. Use the grid to stay within the map."),control:worldNumber(row,"y",0,255,`Draw Point ${row.drawId} Y`)}),detailField({label:"SUB-ID",help:infoHelp("Location discriminator for this world-position record. Several draw points can share one map block, and this byte tells them apart; the game matches it together with the block when deciding which draw point the player is standing on."),control:worldNumber(row,"subId",0,255,`Draw Point ${row.drawId} sub-ID`)})]})],"world-map-detail world-draw-point");
  }
  ...
  function worldSegmentDetail(row){
    ...
    return sharedDetail({...row,name:`WORLD SEGMENT ${row.id}`},null,[
      detailSection({title:"MAP POSITION",body:[detailField({label:"X",help:infoHelp("Read-only X cell coordinate in the world-map grid."),control:readonlyField(row.x)}),detailField({label:"Y",help:infoHelp("Read-only Y cell coordinate in the world-map grid."),control:readonlyField(row.y)}),detailField({label:"REGION ID",help:infoHelp("This region combines with the segment's ground type to choose which world-map encounter rule and encounter group apply here. The rules table is on the Encounters tab."),control:worldNumber(region,"regionId",0,255,`World segment ${row.id} region ID`)})]}),
      detailSection({title:"WMX GEOMETRY",help:infoHelp("Deling proves the group ID at the start of each 0x9000-byte WMX segment. Lexeditor changes only that value and preserves all polygon topology and unknown bytes."),body:[detailField({label:"GROUP ID",help:infoHelp("Stored geometry group for this segment. Its full effect in the game is not established. This is not the encounter group; use the Rules page on the Encounters tab to change battles."),control:worldNumber(row,"groupId",0,4294967295,`World segment ${row.id} group ID`)}),detailField({label:"POLYGONS",help:infoHelp("Number of terrain faces in this segment. This count is read-only."),control:readonlyField(row.polygonCount)}),detailField({label:"GROUND TYPES",help:infoHelp("Terrain codes used by the polygons in this segment. A ground code and the region ID select an encounter rule on the Encounters tab. These are stored codes, not counts."),control:el("span",{class:"world-ground-list"},row.groundTypes.join(", ")||"None")})]}),
  ...
  function renderWorldVisual(){
    ...
    const cells=segments.map(segment=>({id:segment.id,label:`Select world segment ${segment.id}`,selected:segment.id===selected,
      title:`Segment ${segment.id} · region ${worldRow(state.data,"region",segment.id)?.regionId??"?"} · group ${segment.groupId}`}));
  ...
  function worldDetail(row,prefs){
    if(row.kind==="region")return sharedDetail({...row,name:`REGION CELL ${row.id}`},prefs,[detailSection({title:"WORLD REGION",body:[detailField({label:"X",help:infoHelp("Read-only X cell coordinate in the world-map grid."),control:readonlyField(row.x)}),detailField({label:"Y",help:infoHelp("Read-only Y cell coordinate in the world-map grid."),control:readonlyField(row.y)}),detailField({label:"REGION ID",help:infoHelp("This region combines with ground type to choose which world-map encounter rule and encounter group apply in this segment. The rules table is on the Encounters tab."),control:worldNumber(row,"regionId",0,255,"World region ID")})]})],"world-map-detail");
