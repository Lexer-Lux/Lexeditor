  ...
  const worldPropertyHelp={
    ...
    drawPoint:{
      ...
      x:"Packed position byte: its lower seven bits give the block column; its top bit gives "
        +"the odd or even block row. Use the grid for visual placement.",
      y:"Packed row byte. The block row is twice this value plus the top bit of X. "
        +"Use the grid to stay within the map.",
      subId:"Location discriminator for this world-position record. Several draw points can "
        +"share one map block, and this byte tells them apart; the game matches it together "
        +"with the block when deciding which draw point the player is standing on."}};
