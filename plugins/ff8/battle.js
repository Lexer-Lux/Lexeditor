  ...
  function worldSegmentDetail(row){
    ...
    return sharedDetail({...row,name:`WORLD MAP CELL ${row.id}`},null,[
      detailSection({title:"MAP POSITION",body:[detailField({label:"X",help:infoHelp("Read-only X cell coordinate in the world-map grid."),control:readonlyField(row.x)}),detailField({label:"Y",help:infoHelp("Read-only Y cell coordinate in the world-map grid."),control:readonlyField(row.y)}),detailField({label:"REGION CODE",help:infoHelp("The code stored for this cell. It combines with the ground type of the terrain here to choose the world-map encounter rule and encounter group. The rules table is on the Encounters tab."),control:worldNumber(region,"regionId",0,255,`World map cell ${row.id} region code`)})]}),
