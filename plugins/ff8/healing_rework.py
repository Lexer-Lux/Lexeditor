...
# routine's saved-EDI push. The battle record stride is 0xD0 and +0xC0 is the
# effective SPR byte (absolute displacement 0x01D27BD0 after index scaling; the
# vanilla magic routine reads target SPR there at 00491BF4). Lexer's mod doc:
# healing uses spirit, not magic.
...
def build_code_cave() -> bytes:
    """Return `healing = spell power * effective caster SPR`."""
    payload = bytearray()
    ...
    payload += bytes.fromhex("0F B6 80 D0 7B D2 01")  # movzx eax,[eax+SPR]
...
def reworked_healing(spell_power: int, caster_spirit: int) -> int:
    """Mirror the isolated arithmetic core for tests and UI explanations."""
    if isinstance(spell_power, bool) or isinstance(caster_spirit, bool):
        raise ValueError("Spell power and caster SPR must be bytes")
    power = int(spell_power)
    magic = int(caster_spirit)
    if not 0 <= power <= 255 or not 0 <= magic <= 255:
        raise ValueError("Spell power and caster SPR must be from 0 to 255")
    return power * magic
