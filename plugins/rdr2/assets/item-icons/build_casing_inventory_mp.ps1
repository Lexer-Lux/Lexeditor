...
$Build = Join-Path $PSScriptRoot "build_casing_inventory_mp"
$Dds = Join-Path $Build "dds"
$Builder = Join-Path $Build "BuildYtd.exe"
$GtaYtd = Join-Path $Build "inventory_items_mp.ytd"
$FinalYtd = Join-Path $PSScriptRoot "inventory_items_mp.ytd"
...
python (Join-Path $PSScriptRoot "prepare_casing_inventory_mp.py") --output $Dds
...
$converted = Get-ChildItem $Build -Filter "inventory_items_mp*_nya.ytd" |
    Sort-Object LastWriteTime -Descending | Select-Object -First 1
if (!$converted) { throw "CitiCon did not produce an RDR2 _nya.ytd" }
Copy-Item $converted.FullName $FinalYtd -Force
Copy-Item $FinalYtd (Join-Path $Repo "MyOverhaul\stream\inventory_items_mp.ytd") -Force
Write-Output "Built complete Rockstar plus custom INVENTORY_ITEMS_MP replacement: $FinalYtd"
