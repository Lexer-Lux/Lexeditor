...
from .plugin import (DEFAULT_PROJECT_ROOT, MOD_SOURCE_ROOTS, TERRARIA_SAVE_ROOT,
                     TMODLOADER_SAVE_ROOT)
...
def _read_build() -> tuple[bytes, str]:
    ...
    if not target.is_file():
        ...
        # The reader may well have mods installed and running: those are
        # compiled, and this plugin edits sources. Saying how many are there
        # answers "why does it say no mod" without pretending they are absent.
        compiled = sum(
            len(list((root / "Mods").glob("*.tmod"))) if (root / "Mods").is_dir() else 0
            for root in (TMODLOADER_SAVE_ROOT, TERRARIA_SAVE_ROOT / "ModLoader"))
        raise ValueError(
            f"No Terraria mod source project is at {target}. "
            f"Lexeditor looked in: {looked}. A source project is a folder that "
            f"contains build.txt; this installation has {compiled} compiled mods, "
            "and a mod the game runs from Mods or the Workshop is compiled, not "
            "source. Choose Create Mod in the Mod menu, or select the folder "
            "that holds your build.txt.")
